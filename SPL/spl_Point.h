...
void spl_Rotate(spl_Point *p,double Rot,spl_Point *Center,spl_Point *P);
...
public:
    ...
    void Draw(HDC hdc,TColor Color,bool bShowAxis,TColor AxisColor)
    {
		TCanvas *pCanvas = new TCanvas();
        pCanvas->Handle = hdc;
        pCanvas->Pen->Color = Color;
		pCanvas->MoveTo(NW.x,NW.y);
		pCanvas->LineTo(NE.x,NE.y);
		pCanvas->LineTo(SE.x,SE.y);
		pCanvas->LineTo(SW.x,SW.y);
		pCanvas->LineTo(NW.x,NW.y);
		if(bShowAxis)
        {
            pCanvas->Pen->Color = AxisColor;
            pCanvas->MoveTo(left.x,left.y);
            pCanvas->LineTo(left.x+100*k.x,left.y+100*k.y);
            pCanvas->TextOutA(left.x+100*k.x,left.y+100*k.y,"K");

            pCanvas->MoveTo(left.x,left.y);
            pCanvas->LineTo(left.x+100*n.x,left.y+100*n.y);
            pCanvas->TextOutA(left.x+100*n.x,left.y+100*n.y,"N");

            pCanvas->TextOutA(left.x,left.y,"left");
            pCanvas->TextOutA(right.x,right.y,"right");
            pCanvas->TextOutA(bottom.x,bottom.y,"bottom");
            pCanvas->TextOutA(top.x,top.y,"top");

            pCanvas->TextOutA(NW.x,NW.y,"NW");
            pCanvas->TextOutA(NE.x,NE.y,"NE");
            pCanvas->TextOutA(SE.x,SE.y,"SE");
            pCanvas->TextOutA(SW.x,SW.y,"SW");

            pCanvas->MoveTo(NW.x,NW.y);
            pCanvas->LineTo(SE.x,SE.y);
            pCanvas->MoveTo(NE.x,NE.y);
            pCanvas->LineTo(SW.x,SW.y);

            spl_Point C = Center();
            pCanvas->TextOutA(C.x,C.y,"C");
		}
        if(pCanvas)
        {
         	delete pCanvas;
...
/*    
    	double dArea = 0.0;
    	dArea += spl_TriArea(&NW,&NE,&SE);
    	dArea += spl_TriArea(&NW,&SE,&SW);
    	return spl_ABS(dArea);
*/
    	double dArea = 0.0;
    	dArea = spl_PDistance(&NW,&NE) * spl_PDistance(&NW,&SW);
    	return spl_ABS(dArea);
    }
    double Round()
    {
    	double dRound = 0.0;
    	dRound = spl_PDistance(&NW,&NE) + spl_PDistance(&NW,&SW);
    	return spl_ABS(dRound);
    }
    spl_Point Center()
    {
    	spl_Point C;
        C.x = (NW.x+NE.x+SE.x+SW.x)/4.0;
        C.y = (NW.y+NE.y+SE.y+SW.y)/4.0;
    	return C;
    }
};
//---------------------------------------------------------------------------
