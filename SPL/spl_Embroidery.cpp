...
void spl_EmbroidRegion(spl_Region &Region,spl_GlobalStitchPath &GlobalStitchPath)
{
    ...
    switch(nType)
    {
     	case 0:
        {
        	_p0 = splPoint(Frame.left,Frame.bottom);
            _Angle = 0.0;
            break;
        }
     	case 1:
     	...
     	case 2:
        {
     	...
     	case 3:
        {
        	_p0 = splPoint(Frame.right,Frame.bottom);
            _Angle = 3.0*spl_PI/4.0;
            break;
        }
    }
    _K.x = cos(_Angle);
    ...
    {
    	Contour = Region.Contours[i];
    	if(Contour.Points.empty())	continue;
    	OP = Contour.Points[0];
        op = OP;
     	for(UINT j=1;j<Contour.Points.size();j++)
        {
        	P = Contour.Points[j];
            ...
            spl_PVector(&OP,&P,&V);
            Y = spl_PCrossVector(&V,&K);
            X = spl_PDotVector(&V,&K);
            if(spl_ABS(X) < 2.0) continue;
            dTan_t = Y/X;

			for(double t=0.0;t<X;t+=1.0)
            {
                step++;
                if(step < nStep)	continue;
                step = 0;

				p.x = OP.x+t*(K.x+K.y*dTan_t);
				p.y = OP.y+t*(K.y-K.x*dTan_t);
