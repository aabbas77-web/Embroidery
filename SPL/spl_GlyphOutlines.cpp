    ...
    while ((DWORD)lpHeader < (DWORD)(((LPSTR)lpStart) + size) && pt != NULL)
    {
        ...
        // type of contour.
        {
            ...
            for (i = 0; i < cTotal; i++)
            {
                ...
                Contour.Points.push_back(splPoint(pt[i].x,pt[i].y));
            }
            // Draw the contour
            #ifdef spl_EMB_EDITOR
//            spl_DrawPolyline(VL_POLY_BEZIER,true,pt,cTotal);
...
void CharToRegion(HDC hdc,int nCharCode,bool bUnicode,spl_Point &Origin,spl_Region &Region)
{
    ...
    OutlineToRegion(lpHeader,size,Origin,Region);
...
void GetCharacterInfo(HDC hdc,int nCharCode,bool bUnicode,GLYPHMETRICS &GlyphMetrics)
{
    ...
    #ifdef _WIN32
    if(bUnicode)
	    GetGlyphOutlineW(hdc,nCharCode,uFormat,&GlyphMetrics,0,NULL,&FXMatrix);
    else
	    GetGlyphOutline(hdc,nCharCode,uFormat,&GlyphMetrics,0,NULL,&FXMatrix);
    #else
	    GetGlyphOutline(hdc,nCharCode,uFormat,&GlyphMetrics,0,NULL,&FXMatrix);
    #endif// _WIN32
}
