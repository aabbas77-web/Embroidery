...
//---------------------------------------------------------------------------
void spl_MoveCurve(double dx,double dy,spl_Contour &Contour)
{
    if(Contour.Points.empty())
        return;
    int count=Contour.Points.size();
...
void spl_ScaleContour(spl_Contour &iContour,float fXScale,float fYScale,bool Minimize,double Error,spl_Contour &oContour)
{
    ...
    double dW,dH;
    double dw,dh;
	Frame = spl_ComputeFrame(iContour);
    dW = Frame.Width()  * fXScale;
    dH = Frame.Height() * fYScale;
    dw = dW - Frame.Width();
    dh = dH - Frame.Height();
    Frame.left   -= dw/2.0;
    Frame.top    -= dh/2.0;
    Frame.right  += dw/2.0;
    Frame.bottom += dh/2.0;
	spl_ScaleContour(iContour,Frame,Minimize,Error,oContour);
}
//---------------------------------------------------------------------------
/*
void spl_ScaleContour(spl_Contour &iContour,float fXScale,float fYScale,bool Minimize,double Error,spl_Contour &oContour)
...
//---------------------------------------------------------------------------
void spl_Rotate(double Angle,spl_Point &Center,spl_Contour &iContour,spl_Contour &oContour)
{
	oContour.Points.clear();
    if(iContour.Points.empty())  return;
    spl_Point P,NP;
    for(spl_UInt i=0;i<iContour.Points.size();i++)
    {
        P = iContour.Points[i];
        spl_Rotate(&P,Angle,&Center,&NP);
        oContour.Points.push_back(NP);
    }
}
//---------------------------------------------------------------------------
