...
#include <tchar.h>
#include <process.h>
#include "vc_vecad_resource.h"
#include "vc_vecad_VecRes.h"
#include "vc_vecad_Config.h"
#include "bcpp_editor_Mru.h"
#include "bcpp_editor_ExtApp.h"

#define OPEN_VEC_FILE 12593
...
static char* szVecWndClass = "VeCAD_MainWindow";
static int  VecX0 = 0;
static int  VecY0 = 0;
static int  SBarH = 0;  // height of status bar
static char CmdLineArg[256];
...
BOOL CALLBACK fnEnumProc (HWND hwnd,LPARAM lParam)
{
  ...
  buf[5]=0;
  if (strcmp(buf, "VeCAD")==0){
    int len = strlen(szVecWndClass);
    ZeroMemory( buf, sizeof(buf) );
    GetClassName( hwnd, buf, len+1 );
    buf[len] = 0;
    if (strcmp(buf,szVecWndClass)==0){
      ghwMain = hwnd;
      return FALSE;
    }
  }
...
//-----------------------------------------------
void GetFirstArg (const char* cmd, char* arg)
{
  int i,j,len;
  len = strlen(cmd);
  for (i=j=0; i<len; i++){
                     ...
                     int       nCmdShow)
{
  ...
  //--- Test for exist App instance ---
  GetFirstArg( lpCmdLine, CmdLineArg );
  ghwMain = NULL;
  EnumWindows( (WNDENUMPROC)fnEnumProc, 0 );
  if (ghwMain){
    if (strlen(CmdLineArg)>2){
      FILE* df = fopen( vuGetAppDir("\\v_130967.tmp"), "wt" ); //?????
      if (df){
        fprintf( df,"%s\n", CmdLineArg );
        fclose( df );
        PostMessage( ghwMain, WM_USER, OPEN_VEC_FILE, 0 );
      }
    }
    SetForegroundWindow( ghwMain );
    return 0;
  }
  //------------------------------
  ...
  //--- Open VEC drawing if the filename is passed with command line
  if (strlen(CmdLineArg)>2){
//    vlFileOpen( ghwVec, DwgProc, CmdLineArg );
    vlFileOpen( ghwVec, CmdLineArg );
  }else{
    SetCurrentDirectory( vuGetAppDir(NULL) );
  }
...
/*    
  ...
  x = 0;
  y = 0;
  vlToolBarCreate( ghwMain, VL_TB_DRAW, x, y, 30, -1, &w, &h );
  vlToolBarButton( VC_EDIT_ENTPROP );
  vlToolBarButton( VC_EDIT_MOVE );
  vlToolBarButton( VC_EDIT_COPY );
  vlToolBarButton( VC_EDIT_ROTATE );
  vlToolBarButton( VC_EDIT_SCALE );
  vlToolBarButton( VC_EDIT_MIRROR );
  vlToolBarButton( 0 );
  vlToolBarButton( VC_EDIT_ERASE );
  vlToolBarButton( VC_EDIT_EXPLODE );
  y = 0;
  x = x + w;
  vlToolBarCreate( ghwMain, VL_TB_EDIT, x, y, 30, -1, &w, &h );
  x = x + w;
*/
...
//  vlToolBarButton( TBB_GROUP );
//  vlToolBarButton( TBB_UPGROUP );
...
*/
  ...
  ghwVec = vlWndCreate( ghwMain, VL_WS_CHILD|VL_WS_SCROLL|VL_WS_BORDER, 0,0,400,300); //, DwgProc );
  if (ghwVec){
    TCHAR szStr[128];
...
//-----------------------------------------------
void ResizeVecWindow (int w, int h)
{
  if (w>0 && h>0){
...
void OpenVecFile ()
{
  char szTmpFile[256], szVecFile[256];
  strcpy( szTmpFile, vuGetAppDir("\\v_130967.tmp") );
  FILE* df = fopen( szTmpFile, "rt" );
  if (df){
    ZeroMemory( szVecFile, 255 );
    fscanf( df,"%s", szVecFile );
    fclose( df );
//    vlFileOpen( ghwVec, DwgProc, szVecFile );
    vlFileOpen( ghwVec, szVecFile );
  }
...
static LRESULT CALLBACK WndProc (HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
  switch( message ){
    ...
    case WM_SIZE:
      // Resize VeCAD window
      ResizeVecWindow( LOWORD(lParam), HIWORD(lParam) );
      break;
...
{
  vlFileNew( ghwVec, _T("") );
//  vlFileNew( ghwVec, DwgProc, _T("") );
/*
  int iDwg = vlDwgCreate( ghwVec, DwgProc );
  if (iDwg>=0){
    vlClear( TRUE );
//    vlPropPutInt( VD_DWG_AUTOSELWIN, -1, 0 );
    vlRedraw();
    vlSetFocus();
  }
*/
...
void OnCmFileOpen () 
{
  vlFileOpen( ghwVec, _T("") );
//  vlFileOpen( ghwVec, DwgProc, _T("") );
/*
  vlPropPutInt( VD_DWG_READONLY, -1, 1 );
  vlExecute(VC_ZOOM_PAN);
//  vlPropPutInt( VD_DWG_AUTOSELPNT, -1, 0 );
//  vlPropPutInt( VD_DWG_AUTOSELRECT, -1, 0 );
*/
/*
  int iDwg = vlDwgCreate( ghwVec, DwgProc );
  if (iDwg>=0){
    if (vlFileOpen( VL_FILE_VEC, NULL )==FALSE){
      vlDwgDelete( iDwg );
    }
  }
*/
...
//-----------------------------------------------
void OnOpenDesignTemplate ()
{
//  vlFileNew( ghwVec, DwgProc, _T("e:\\!oks2\\vecad2\\design0.vec") );
//  vlFileNew( ghwVec, DwgProc, _T("design0.vec") );
  vlFileNew( ghwVec, _T("design0.vec") );
}
//-----------------------------------------------
void OnOpenCadTable ()
{
//  vlFileNew( ghwVec, DwgProc, _T("table0.vec") );
  vlFileNew( ghwVec, _T("table0.vec") );
}

//-----------------------------------------------
void OnHelp ()
{
  ::WinHelp( ghwMain, _T("vecapi.hlp"), HELP_CONTEXT, 1 );
}
...
static bool OnCommand (HWND hWnd, int Cmd)
{
  ...
  switch( Cmd ){
    ...
    case CM_APP_ABOUT: DlgAbout(hWnd); break;
    case CM_HELP:      OnHelp(); break;
    ...
    case VC_IMPORT_DP1:  OnCmImportDp1(); break;
    case VC_IMPORT_HPGL: OnCmImportHpgl(); break;
    ...
    case VC_SEL_BYPOINT  : OnCmSelByPoint(); break; 
    case VC_SEL_BYRECT   : OnCmSelByRect(); break; 
    case VC_SEL_BYPOLYGON: OnCmSelByPolygon(); break; 

    case VC_INS_GLASS: OnCmInsGlass(); break;
    case ID_GLASS_OPENDESIGNTEMPLATE:
      OnOpenDesignTemplate();
      break;
    case ID_GLASS_OPENCADTABLE:
      OnOpenCadTable();
      break;
