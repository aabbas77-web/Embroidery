...
type
  ...
  private
    ...
    FPosition: Single;
    FIncrement: Integer;
    FOnGetWindowSize: TRBGetSizeEvent;
  ...
  protected
    ...
    function DoGetWindowSize: Integer; virtual;
  ...
  public
    ...
    property OnGetWindowSize: TRBGetSizeEvent read FOnGetWindowSize write FOnGetWindowSize;
  end;
...
begin
  ...
  begin
    Frame3D(Canvas, R, CLo, clBlack, 1);
...
function TCustomRangeBar.DoGetWindowSize: Integer;
begin
  if Kind = sbHorizontal then Result := Width
  else Result := Height;
  if Assigned(FOnGetWindowSize) then FOnGetWindowSize(Self, Result);
  OldSize := Result;
end;
...
var
  Delta: Single;
  WinSz, ClientSz, HandleSz: Integer;
begin
  inherited;
  if MouseInfo = miHandle then
  begin
...
procedure TCustomRangeBar.SetIncrement(Value: Integer);
begin
