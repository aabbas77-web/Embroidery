...
uses
  Windows, SysUtils, Classes, Controls, Forms, Graphics, G32, Dialogs;
...
type
  ...
  TDragState32 = (dsNone, dsMove, dsSizeL, dsSizeT, dsSizeR, dsSizeB,
  ...
  protected
    ...
    function  DoHitTest(X, Y: Integer): Boolean; override;
    procedure DoResizing(var OldLocation, NewLocation: TFloaTRect; DragState: TDragState32; Shift: TShiftState); virtual;
    procedure DoSetLocation(const NewLocation: TFloaTRect); override;
    function  GetDragState(X, Y: Integer): TDragState32; virtual;
...
uses TypInfo, G32_Image, G32_LowLevel;
...
begin
  DsTRect := Rect32(GetAdjustedLocation);
  SrcRect := Rect32(0, 0, Bitmap.Width, Bitmap.Height);
...
procedure TRubberbandLayer.DoResizing(var OldLocation,
  NewLocation: TFloaTRect; DragState: TDragState32; Shift: TShiftState);
begin
...
function TRubberbandLayer.GetDragState(X, Y: Integer): TDragState32;
var
  R: TRect;
...
begin
  ...
  else if dh_center and PtInRect(R, Point32(X, Y)) then Result := dsMove;
end;
...
const
  CURSOR_ID: array [TDragState32] of TCursor = (crDefault, crDefault, crSizeWE,
...
var
  ...
  begin
    Buffer.FillRectS(X - FHandleSize, Y - FHandleSize, X + FHandleSize, Y + FHandleSize, clWhite32);
    Buffer.FrameRectS(X - FHandleSize, Y - FHandleSize, X + FHandleSize, Y + FHandleSize, clBlack32);
  end;
