...
uses
  ...
  Menus, ExtCtrls, JPeg, ExtDlgs, StdCtrls, G32, G32_Image, G32_Layers,
  G32_RangeBars, G32_Filters, G32_Transforms;
...
type
  ...
  protected
    ...
    procedure RBResizing(Sender: TObject; const OldLocation: TFloaTRect; var NewLocation: TFloaTRect; DragState: TDragState32; Shift: TShiftState);
    ...
    procedure PaintTextHandler(Sender: TObject; Buffer: TBitmap32);
  public
    Text:String;
...
uses NewImageUnit, RGBALoaderUnit, Math;
...
begin
  ...
  if Selection is TBitmapLayer then
    ...
    begin
      T := TBitmap32.Create;
      T.Assign(Bitmap);
      with Rect32(Location) do
        Bitmap.SetSize(Right - Left, Bottom - Top);
      T.StretchFilter := sfLinear;
      T.DrawMode := dmOpaque;
      T.DrawTo(Bitmap, Rect32(0, 0, Bitmap.Width, Bitmap.Height));
      T.Free;
...
procedure TMainForm.RBResizing(Sender: TObject;
  ...
  DragState: TDragState32; Shift: TShiftState);
var
  w, h, cx, cy: Single;
  nw, nh, r: Single;
...
procedure TMainForm.PaintTextHandler(Sender: TObject; Buffer: TBitmap32);
var
  Cx, Cy: Single;
  W2, H2: Single;
  W, H: Single;
...
begin
  if Sender is TPositionedLayer then
    ...
    begin
      ...
      Buffer.Font.Size:=Round(W);
      Size:=Buffer.TextExtent(Text);
      Buffer.TextOut(Round(Cx-Size.cx/2),Round(Cy-Size.cy/2),Text);
//      Buffer.RenderText(Round(Left),Round(Top),Text,1,clRed32);
    end;
end;
