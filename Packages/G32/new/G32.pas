...
uses
  Windows, Classes, SysUtils, Messages, Controls, Graphics, Dialogs;
...
function Point32(X, Y: Integer): TPoint; overload;
function Point32(const FP: TFloatPoint): TPoint; overload;
function Point32(const FXP: TFixedPoint): TPoint; overload;
...
function Rect32(L, T, R, B: Integer): TRect; overload;
function Rect32(const FR: TFloaTRect; Rounding: TRectRounding = rrClosest): TRect; overload;
function Rect32(const FXR: TFixedRect; Rounding: TRectRounding = rrClosest): TRect; overload;
...
procedure OffseTRect(var FR: TFloaTRect; Dx, Dy: Single); overload;
...
{ Stretch filters }
  TStretchFilter = (sfNearest, sfLinear, sfLinear2, sfSpline);
...
type
  ...
  public
    ...
    function SetSize(Source: TPersistent): Boolean; overload;
    function SetSize(NewWidth, NewHeight: Integer): Boolean; overload; virtual;
...
uses G32_Blend, G32_Transforms, G32_LowLevel, G32_Filters, Math, TypInfo,
  Clipbrd, G32_DrawingEx;
...
begin
  ...
  Result :=
    (Color32 and $00FF0000) shr 16 or
...
begin
  ...
  L := High(Colors) + 1;
  SetLength(Result, L);
  Move(Colors[0], Result[0], L shl 2);
end;
...
function FixedMul(A, B: TFixed): TFixed;
begin
  Result := MulDiv(A, B, $10000);
end;

function FixedDiv(A, B: TFixed): TFixed;
begin
  Result := MulDiv(A, $10000, B);
end;

{ Points }

function Point32(X, Y: Integer): TPoint;
begin
  Result.X := X;
  Result.Y := Y;
end;

function Point32(const FP: TFloatPoint): TPoint;
begin
  Result.X := Round(FP.X);
  Result.Y := Round(FP.Y);
end;

function Point32(const FXP: TFixedPoint): TPoint;
begin
...
function Rect32(L, T, R, B: Integer): TRect;
begin
  with Result do
  begin
    Left := L;
...
function Rect32(const FR: TFloaTRect; Rounding: TRectRounding): TRect;
begin
  with FR do
    case Rounding of
      rrClosest:
        Result := Rect32(Round(Left), Round(Top), Round(Right), Round(Bottom));

      rrInside:
        begin
          Result := Rect32(Ceil(Left), Ceil(Top), Floor(Right), Floor(Bottom));
          if Result.Right < Result.Left then Result.Right := Result.Left;
          if Result.Bottom < Result.Top then Result.Bottom := Result.Top;
        end;

      rrOutside:
        Result := Rect32(Floor(Left), Floor(Top), Ceil(Right), Ceil(Bottom));
    end;
end;

function Rect32(const FXR: TFixedRect; Rounding: TRectRounding): TRect;
begin
  with FXR do
    case Rounding of
      rrClosest:
...
begin
  ...
  Result := (Dst.Right >= Dst.Left) or (Dst.Bottom >= Dst.Top);
  if not Result then
  begin
...
function TCustomMap.SetSize(Source: TPersistent): Boolean;
begin
...
function TBitmap32.SetSize(NewWidth, NewHeight: Integer): Boolean;
begin
  Result := False;
...
//AliSoft 28.03.2003      FHandle := CreateDIBSection(0, FBitmapInfo, DIB_RGB_COLORS, Pointer(FBits), 0, 0);
      FHandle := CreateDIBSection(FHDC, FBitmapInfo, DIB_RGB_COLORS, Pointer(FBits), 0, 0);
  ...
  try
    ...
    begin
      ...
      try
        ...
        TGraphicAccess(Source).Draw(Canvas, Rect32(0, 0, Width, Height));
        ResetAlpha;
      finally
        Canvas.Free;
      end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          try
            Canvas.Handle := Self.Handle;
            TGraphicAccess(Graphic).Draw(Canvas, Rect32(0, 0, Width, Height));
            ResetAlpha;
          finally
            Canvas.Free;
          end;
    ...
    begin
      ...
      try
        ...
        try
          Canvas.Handle := Self.Handle;
          TGraphicAccess(Picture.Graphic).Draw(Canvas, Rect32(0, 0, Width, Height));
          ResetAlpha;
        finally
          Canvas.Free;
        end;
...
begin
  ...
  BlockTransfer(Dst, 0, 0, Self, Rect32(0, 0, Width, Height), DrawMode);
  Dst.Changed;
end;
...
begin
  if Empty or Dst.Empty then Exit;
  BlockTransfer(Dst, DstX, DstY, Self, Rect32(0, 0, Width, Height), DrawMode);
  Dst.Changed;
end;
...
begin
  if Empty or Dst.Empty then Exit;
  StretchTransfer(Dst, DsTRect, Self, Rect32(0, 0, Width, Height),
    StretchFilter, DrawMode);
  Dst.Changed;
end;
...
begin
  if Empty then Exit;
  StretchBlt(hDst, DsTRect.Left, DsTRect.Top, DsTRect.Right - DsTRect.Left,
    DsTRect.Bottom - DsTRect.Top, Handle, SrcRect.Left, SrcRect.Top,
    SrcRect.Right - SrcRect.Left, SrcRect.Bottom - SrcRect.Top, SRCCOPY);
end;
...
begin
  try
    ...
    begin
      ...
      else if Dy < 0 then VertLine(X1, Y2, Y1 - 1, Value);
    ...
    begin
      ...
      else HorzLine(X2, Y1, X1 - 1, Value);
      if L then Pixel[X2, Y2] := Value;
      Exit;
    end;
...
begin
  try
    ...
    begin
      ...
      else if Dy < 0 then VertLineT(X1, Y2, Y1 - 1, Value);
    ...
    begin
      ...
      else HorzLineT(X2, Y1, X1 - 1, Value);
      if L then SetPixelT(X2, Y2, Value);
      Exit;
    end;
    ...
    begin
      ...
      try
        ...
        try
          ...
          ScreenCanvas.Font.Size := Font.Size shl AALevel;
          Sz := ScreenCanvas.TextExtent(Text + ' ');
          Sz.Cx := (Sz.cx shr AALevel + 1) shl AALevel;
          B2 := TBitmap32.Create;
          try
            B2.SetSize(Sz.Cx, Sz.Cy);
            B2.Clear(0);
            B2.Font := ScreenCanvas.Font;
...
begin
  ...
  if FillBack then
  begin
    R := Rect32(0, 0, Width, Height);
    OffseTRect(R, Dx, Dy);
    IntersecTRect(R, R, Rect32(0, 0, Width, Height));
    if R.Top > 0 then FillRect(0, 0, Width - 1, R.Top - 1, FillColor)
    else if R.Top < 0 then FillRect(0, R.Bottom, Width - 1, Height - 1, FillColor);
    if R.Left > 0 then FillRect(0, R.Top, R.Left - 1, R.Bottom - 1, FillColor)
    else if R.Left < 0 then FillRect(R.Right, R.Top, Width - 1, R.Bottom - 1, FillColor);
  end;
