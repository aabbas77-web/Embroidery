...
procedure TCustomPaintBox32.CreateParams(var Params: TCreateParams);
begin
...
begin
  DrawFocusRect(Dest.Handle, Rect32(0, 0, Width, Height));
end;
...
begin
  with GetViewporTRect do
  begin
    HScroll.BoundsRect := Rect32(Left, Bottom, Right, Height);
    VScroll.BoundsRect := Rect32(Right, Top, Width, Bottom);
    HScroll.Repaint;
    VScroll.Repaint;
  end;
end;
...
var
  Sz: Integer;
begin
  Result := Rect32(0, 0, Width, Height);
  Sz := GetScrollBarSize;
  Dec(Result.Right, Sz);
...
begin
  inherited;
  if IsSizeGripVisible then
    DrawFrameControl(Canvas.Handle, GetSizeGripRect, DFC_SCROLL, DFCS_SCROLLSIZEGRIP)
  else
  begin
...
begin
  ...
  VScroll.Range := Sz.Cy;
  I := GetScrollBarSize;
  HScroll.Window := Width - I;
  VScroll.Window := Height - I;
end;
