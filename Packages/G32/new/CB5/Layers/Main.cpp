...
void __fastcall TFormMain::LayerMouseDown(TObject *Sender,TMouseButton Buttons,TShiftState Shift,int X,int Y)
{
 if(Sender)
 {
  ...
  if(((TBitmapLayer *)Sender))    
   Panel1->Color=WinColor(((TBitmapLayer *)Sender)->Bitmap->Pixel[X][Y]);
 }
}
//---------------------------------------------------------------------------
...
void __fastcall TFormMain::Quantize1Click(TObject *Sender)
{
 if(!OpenPictureDialog1->Execute())
  return;
 ...
 TColors Colors;
 AnsiString S;
 S=InputBox("Quantize","Enter Colors Count:","8");
 int ColorCount=StrToInt(S);
 Graphics::TBitmap *pBMP=new Graphics::TBitmap();

 pBMP->PixelFormat=pf32bit;
 pBMP->Width=B->Width;
 pBMP->Height=B->Height;
 BitBlt(pBMP->Canvas->Handle,0,0,pBMP->Width,pBMP->Height,
        B->Handle,0,0,SRCCOPY);

 wuReduce(pBMP,ColorCount,Colors);
 B->Assign(pBMP);
 Temp->Assign(pBMP);
 ...
 int H=pBMP->Height;
 TColor32 TransWhite;
 TransWhite=Color32(255,255,255,0);
 ...
 for(int i=0;i<ColorCount;i++)
 {
  ...
  for(int Y=0;Y<H;Y++)
  {
   PRGB1=(TColor *)pBMP->ScanLine[Y];
   PRGB2=(TColor32 *)B->ScanLine[Y];
   for(int X=0;X<W;X++)
   {
    if(PRGB1[X]==Colors[i])
     PRGB2[X]=Color32(PRGB1[X]);
   }
  }
...
//---------------------------------------------------------------------------
void __fastcall TFormMain::CreateBitmapLayer(TBitmap32 *pBMP)
{
 TBitmapLayer *B;
 TPoint P;
 Single W, H;
