...
begin
  ...
  SrcX := SrcRect.Left;
  SrcY := SrcRect.Top;
  IntersecTRect(S, SrcRect, Rect32(0, 0, Src.Width, Src.Height));
  OffseTRect(S, DstX - SrcX, DstY - SrcY);
  IntersecTRect(D, S, Rect32(0, 0, Dst.Width, Dst.Height));
  ...
  try
    ...
    begin
      ...
      begin
        Ps := Src.PixelPtr[D.Left + SrcX - DstX, J + SrcY - DstY];
        Pd := Dst.PixelPtr[D.Left, J];
        for I := 0 to N - 1 do CombineCallBack(Ps^, Pd^, MstrAlpha);
      end;
  finally
    EMMS;
  end;
...
begin
  ...
  IntersecTRect(R, DsTRect, Rect32(0, 0, Dst.Width, Dst.Height));
...
begin
  ...
  Scale := (DstWidth - 1) / (SrcHi - SrcLo - 1);
...
begin
  ...
  begin
    ...
    else if CombineOp = dmBlend then
...
begin
  ...
  // clip SrcRect
  IntersecTRect(SrcRectI, Rect32(Transformation.SrcRect),
    Rect32(0, 0, Src.Width - 1, Src.Height - 1));

  with Transformation.SrcRect do
  begin
    IntersecTRect(SrcRect256, Rect32(Round(Left * 256), Round(Top * 256),
      Round(Right * 256), Round(Bottom * 256)), Rect32(0, 0, (Src.Width - 1) * 256,
      (Src.Height - 1) * 256));
  end;

  // clip DsTRect
  R := Transformation.GetTransformedBounds;
  IntersecTRect(DsTRect, R, Rect32(0, 0, Dst.Width - 1, Dst.Height - 1));
  if IsRectEmpty(DsTRect) then Exit;
