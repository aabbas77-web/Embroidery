...
begin
  ...
  SFX := MulDiv((R.Left - DstRect.Left) * SrcW, 65536, DstW);
  SFY := MulDiv((R.Top - DstRect.Top) * SrcH, 65536, DstH);
  ...
  begin
    SrcP := Src.PixelPtr[SrcRect.Left, SrcYYYY shr 16]; // source scanline
...
begin
  ...
  if StretchFilter in [sfNearest, sfLinear] then FilterWidth := 1.1
  ...
  try
    ...
    begin
      ...
      begin
        ...
        if Ca > $FFFFFF then Ca := $FF0000 else
        Ca := Ca and $00FF0000;
        Cr := Cr and $00FF0000;
        Cg := Cg and $00FF0000;
        Cb := Cb and $00FF0000;
