...
function Rect(L, T, R, B: Integer): TRect; overload;
function Rect(const FR: TFloatRect; Rounding: TRectRounding = rrClosest): TRect; overload;
function Rect(const FXR: TFixedRect; Rounding: TRectRounding = rrClosest): TRect; overload;
...
function Rect(L, T, R, B: Integer): TRect;
begin
  with Result do
  begin
    Left := L;
...
function Rect(const FR: TFloatRect; Rounding: TRectRounding): TRect;
begin
  with FR do
    case Rounding of
      rrClosest:
      ...
      rrInside:
        begin
          Result := Rect(Ceil(Left), Ceil(Top), Floor(Right), Floor(Bottom));
...
function Rect(const FXR: TFixedRect; Rounding: TRectRounding): TRect;
begin
  with FXR do
    case Rounding of
      rrClosest:
...
begin
  if Empty or Dst.Empty then Exit;
  StretchTransfer(Dst, DstRect, Self, Rect(0, 0, Width, Height),
...
begin
  if X1 = X2 then
    ...
    begin
      ...
      HorzLineTSP(X1, Y1, X2 + sx);
      VertLineTSP(X2, Y1, Y2 + sy);
      HorzLineTSP(X2, Y2, X1 - sx);
      VertLineTSP(X1, Y2, Y1 - sy);
    end;
  Changed;
end;
...
begin
  ...
  try
    ...
    begin
      B2 := TBitmap32.Create;
      try
        // just need some DC here to get font information
        B2.SetSize(1, 1); 
        B2.Font := Font;
        B2.Font.Size := Font.Size shl AALevel;
        Sz := B2.TextExtent(Text + ' ');

        // now set it to the normal size
...
begin
  ...
  if (Abs(Dx) >= Width) or (Abs(Dy) >= Height) and FillBack then
  begin
    Clear(FillColor);
    Exit;
  end;
