...
/** CTimeDate Timer functionality and time and date functions,
	...
	@version 0.98,  by edr@mip.sdu.dk
...
class CTimeDate
{
	...
	public: // methods
		...
		/** Returns the actual date in the parameter "Date" in
			the format: Weekday Month Day. Deletes previous allocated memory for Time (if any)
			and allocates new memory.
			@return false if failed, otherwise true */
		bool GetDate(std::string& str);
		...
		/** Makes the timer run again after a call to StopTimer()
			@param false if timer was not on hold, then doing nothing */
		bool TimerContinue();
