...
class CPoint3D
{
	...
	public: // methods
		...
		template<class POLY> inline CPoint3D(const CPoint3D<POLY>& P3D){m_x=P3D.GetX();m_y=P3D.GetY();m_z=P3D.GetZ();}
		 /// copy constructor
...
//	double x = m_x*cosb + m_y*sinb*sina - m_z*sinb*cosa;
//	double y = m_y*cosa + m_z*sina;
//	double z = m_x*sinb - m_y*cosb*sina + m_z*cosb*cosa;
	m_x=(T)x;
	m_y=(T)y;
	m_z=(T)z;
}
