...
/** CPoint2D template class with 2-dimensional points,
	...
	@see CPoint3D
	@version 1.45,  by edr@mip.sdu.dk
...
class CPoint2D
{
	...
	public: // methods
		...
		template<class POLY> inline CPoint2D(const CPoint2D<POLY>& P2D){m_x=P2D.GetX();m_y=P2D.GetY();}
		/// copy constructor
		...
		template<class POLY> inline CPoint2D<T>& operator=(const CPoint2D<POLY>& P2D){m_x=P2D.GetX();m_y=P2D.GetY();return *this;};
		/// overloading += operator with template conversion
		template<class POLY> inline CPoint2D<T>& operator+=(const CPoint2D<POLY>& P2D){m_x+=P2D.GetX();m_y+=P2D.GetY();return *this;};
		/// overloading -= operator with template conversion
		template<class POLY> inline CPoint2D<T>& operator-=(const CPoint2D<POLY>& P2D){m_x-=P2D.GetX();m_y-=P2D.GetY();return *this;};
		...
		//@{
		/// overloading == operator with template conversion
		template<class POLY> inline bool operator==(const CPoint2D<POLY>& P2D) const{return ((m_x==P2D.GetX()) && (m_y==P2D.GetY()));}
		/// overloading != operator with template conversion
		template<class POLY> inline bool operator!=(const CPoint2D<POLY>& P2D) const{return ((m_x!=P2D.GetX()) || (m_y!=P2D.GetY()));}
		/// overloading < operator with template conversion. First comparison on x, if equal comparison on y is performed.
		template<class POLY> inline bool operator<(const CPoint2D<POLY>& P2D) const{if (m_x==P2D.GetX()){return (m_y<P2D.GetY());}else{return (m_x<P2D.GetX());}}
		/// overloading <= operator with template conversion. First comparison on x, if equal comparison on y is performed.
		template<class POLY> inline bool operator<=(const CPoint2D<POLY>& P2D) const{if (m_x==P2D.GetX()){return (m_y<=P2D.GetY());}else{return (m_x<P2D.GetX());}}
		/// overloading < operator with template conversion. First comparison on x, if equal comparison on y is performed.
		template<class POLY> inline bool operator>(const CPoint2D<POLY>& P2D) const{if (m_x==P2D.GetX()){return (m_y>P2D.GetY());}else{return (m_x>P2D.GetX());}}
		...
		//@{
		/// overloading + operator with template conversion
		template<class POLY> inline CPoint2D<T> operator+(const CPoint2D<POLY>& P2D) const {return CPoint2D<T>(m_x+P2D.GetX(),m_y+P2D.GetY());}
		/// overloading - operator with template conversion
		template<class POLY> inline CPoint2D<T> operator-(const CPoint2D<POLY>& P2D) const {return CPoint2D<T>(m_x-P2D.GetX(),m_y-P2D.GetY());}
		/** overloading * operator with template conversion
			 @param Factor Multiply each dimension with this value */
		...
		/** Rotates this point Rad radians anti clock wise about origo in an image
		...
		/** Rotates this point PI radians about origo. It is a fast version of
...
//template <class T>
//inline void CPoint2D<T>::SkipSpaceAndComments(std::istream& s)
//{
//	while(isspace(s.peek()))
//		s.get();
//	while (s.peek()=='#')
