...
#define k_AddPoint(pPixelGroups,pThisRow,s,r) pPixelGroups->ppNext[pPixelGroups->pPixelGroup[s].Last.x][pPixelGroups->pPixelGroup[s].Last.y]=r;pPixelGroups->pPixelGroup[s].Last;pPixelGroups->pPixelGroup[s].NumberOfPixels++;pPixelGroups->pPixelGroup[s].Last=r;pThisRow[r.x]=s;
...
bool k_DeriveBlobs(TImage* pSource, TPixelGroups* pPixelGroups,
                    COLORTYPE Color, UINT8 Threshold, CONNECTIVITY Connected)
{
	T2DInt p;
	...
	if (pSource->Origin==EMPTY)
	{
		...
		k_ShowMessage(IPL_ERROR,&str,"k_DeriveBlobs() Source image is empty");
	    k_EmptyString(&str);
		return false;
	}
	else if ((pSource->Bits!=1) && (pSource->Bits!=8))
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_DeriveBlobs() Source image is %d b/p (must be 1 or 8 b/p)",
			pSource->Bits);
	    k_EmptyString(&str);
		return false;
	}
					 ...
					 UINT8 Threshold, CONNECTIVITY Connected)
{
	...
	if (pSource->Origin==EMPTY)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_BlobExpansion() Source image is empty");
	    k_EmptyString(&str);
		return false;
	}
	else if ((pSource->Bits!=1) && (pSource->Bits!=8))
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_BlobExpansion() Source image is %d b/p (must be 1 or 8 b/p)",
			pSource->Bits);
	    k_EmptyString(&str);
		return false;
	}
	...
	{
		/* must be 8 b/p */
		if (k_GetPixel(Seed.x,Seed.y,pSource)>Threshold)
			ColorTp=HIGHCOLOR;
		else
			ColorTp=LOWCOLOR;
		if (pSource->BorderSize==0)
								...
								COLORTYPE* pSurroundedColor)
{
	...
	if (pSource->Origin==EMPTY)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_WARNING,&str,"k_FindAndFollowHighContour() Source image is empty - doing nothing");
	    k_EmptyString(&str);
		return false;
	}
	if (pSource->Bits==24)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_FindAndFollowHighContour() Source image is 24 b/p - not supported");
	    k_EmptyString(&str);
		return false;
	}
		...
		(SearchDirection!=WEST))
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_FindAndFollowHighContour() Wrong search direction");
	    k_EmptyString(&str);
		return false;
	}
	...
	if (!found)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_FindAndFollowHighContour() No contour found");
		k_EmptyString(&str);
		if (ResetBorder==true)
			k_SetBorderSize(0,pSource);
		return false;
								...
								COLORTYPE* pSurroundedColor)
{
	...
	if (!found)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_FindAndFollowLowContour() No contour found");
		k_EmptyString(&str);
		if (ResetBorder==true)
			k_SetBorderSize(0,pSource);
		return false;
											 ...
											 COLORTYPE* pSurroundedColor)
{
	...
	if (pSource->Origin==EMPTY)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_WARNING,&str,"k_FindAndFollowHighContourWithThreshold() Source image is empty - doing nothing");
		k_EmptyString(&str);
		return false;
	}
	if (pSource->Bits==24)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_FindAndFollowHighContourWithThreshold() Source image is 24 b/p - not supported");
		k_EmptyString(&str);
		return false;
	}
	if ((SearchDirection!=NORTH) && (SearchDirection!=EAST) && (SearchDirection!=SOUTH) &&
