...
bool k_DilateFast(TImage* pImg,unsigned short q10,unsigned short q20,unsigned short q30)
{
	...
	if (pImg->Bits!=1)
	{
		...
		k_ShowMessage(IPL_ERROR,&str,"k_DilateFast() Image must be 1 b/p (current is %d b/p)",pImg->Bits);
		k_EmptyString(&str);
		return false;
	}
	if (pImg->Origin==EMPTY)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_WARNING,&str,"k_DilateFast() Source image is empty - doing nothing");
		k_EmptyString(&str);
		return false;
	}
	...
	/* add comment to history */
	{
		...
		k_SprintfString(&History,"%sk_DilateFast(q10=%d,q20=%d,q30=%d)",ipl_HistoryIndent,q10,q20,q30);
...
bool k_ErodeFast(TImage *pImg,unsigned short q10,unsigned short q20,unsigned short q30)
{
	...
	if (pImg->Bits!=1)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_ErodeFast() Image must be 1 b/p (current is %d b/p)",pImg->Bits);
		k_EmptyString(&str);
		return false;
	}
	if (pImg->Origin==EMPTY)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_WARNING,&str,"k_ErodeFast() Source image is empty - doing nothing");
		k_EmptyString(&str);
		return false;
	}
    k_InitImage(&Img1); /* always initialise the TImage structure! */
	...
	/* add comment to history */
	{
		TString History;
		k_InitString(&History);
		k_SprintfString(&History,"%sk_ErodeFast(q10=%d,q20=%d,q30=%d)",ipl_HistoryIndent,q10,q20,q30);
		k_AppendTextIPL(&(pImg->History),History.pChars);
		k_EmptyString(&History);
		k_IplStartHistoryMacro();
	}
...
{ 
	...
	if (pImg->Origin==EMPTY)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_WARNING,&str,"k_Erode() Source/target image is empty - doing nothing");
		k_EmptyString(&str);
		return false;
	}
	if (pMask->Origin==EMPTY)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_WARNING,&str,"k_Erode() Mask image is empty - doing nothing");
		k_EmptyString(&str);
		return false;
	}
	if (pImg->Bits==8)
	...
	else
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_Erode() Source/target image must be 1 or 8 b/p");
		k_EmptyString(&str);
		return false;
	}
	w=pMask->Width;
	...
	/* add comment to history */
	{
		TString History;
		k_InitString(&History);
		k_SprintfString(&History,"%sk_Erode() with dimensions (%d*%d)",ipl_HistoryIndent,pMask->Width,pMask->Height);
		k_AppendTextIPL(&(pImg->History),History.pChars);
		k_EmptyString(&History);
		k_IplStartHistoryMacro();
	}
...
{ /* Similar to Erode */
	...
	if (pImg->Origin==EMPTY)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_WARNING,&str,"k_Dilate() Source/target image is empty - doing nothing");
		k_EmptyString(&str);
		return false;
	}
	if (pMask->Origin==EMPTY)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_WARNING,&str,"k_Dilate() Mask image is empty - doing nothing");
		k_EmptyString(&str);
		return false;
	}
	if (pImg->Bits==8)
	...
	else
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_Dilate() Source/target image must be 1 or 8 b/p");
		k_EmptyString(&str);
		return false;
	}
	...
	/* add comment to history */
	{
		TString History;
		k_InitString(&History);
		k_SprintfString(&History,"%sk_Dilate() with dimensions (%d*%d)",ipl_HistoryIndent,pMask->Width,pMask->Height);
		k_AppendTextIPL(&(pImg->History),History.pChars);
		k_EmptyString(&History);
		k_IplStartHistoryMacro();
	}
...
int k_Thinning(TImage* pImg)
{
	if (pImg->Origin==EMPTY)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_WARNING,&str,"k_Thinning() Image is empty - doing nothing");
		k_EmptyString(&str);
		return -1;
	}
	else if (pImg->Bits!=1)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_WARNING,&str,"k_Thinning() Image is %d b/p, must be 1 b/p",pImg->Bits);
		k_EmptyString(&str);
		return -1;
	}
	else
	{
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%sk_Thinning()",ipl_HistoryIndent);
			k_AppendTextIPL(&(pImg->History),History.pChars);
			k_EmptyString(&History);
			k_IplStartHistoryMacro();
		}
...
int k_Skeletonizing(TImage* pImg)
{
	if (pImg->Origin==EMPTY)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_WARNING,&str,"k_Skeletonizing() Image is empty - doing nothing");
		k_EmptyString(&str);
		return -1;
	}
	else if (pImg->Bits!=1)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_WARNING,&str,"k_Skeletonizing() Image is %d b/p, must be 1 b/p",pImg->Bits);
		k_EmptyString(&str);
		return -1;
	}
	else /* perform skeletonizing */
	{
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%sk_Skeletonizing()",ipl_HistoryIndent);
			k_AppendTextIPL(&(pImg->History),History.pChars);
			k_EmptyString(&History);
			k_IplStartHistoryMacro();
		}
...
int k_SkeletonZhou(TImage* pImg, bool PreserveCorners)
{
	if (pImg->Origin==EMPTY)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_WARNING,&str,"k_SkeletonZhou() Image is empty - doing nothing");
		k_EmptyString(&str);
		return -1;
	}
	else if (pImg->Bits!=1)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_WARNING,&str,"k_SkeletonZhou() Image is %d b/p, must be 1 b/p",pImg->Bits);
		k_EmptyString(&str);
		return -1;
	}
	else /* perform skeletonizing with the Zhou algorithm */
	{
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%sk_SkeletonZhou()",ipl_HistoryIndent);
			k_AppendTextIPL(&(pImg->History),History.pChars);
			k_EmptyString(&History);
			k_IplStartHistoryMacro();
		}
