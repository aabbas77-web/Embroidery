...
T2DFloat k_DeriveCenterOfMassBinary(const TPixelGroup* pPixelGroup)
{
	...
	if ((pPixelGroup==NULL) || (pPixelGroup->NumberOfPixels==0))
	{
		...
		k_ShowMessage(IPL_ERROR,&str,"k_DeriveCenterOfMassBinary() Given pixelgroup is either a NULL-"
...
/* If MaxOrder>2 up to fourth order moments are derived. Otherwise up to second order. */
{
	...
	if ((pPixelGroup==NULL) || (pMom==NULL))
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_DeriveMomentsGrayTone() One of the parameters "
	...
	if (pPixelGroup->pColor==NULL)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_DeriveMomentsGrayTone() The group contains no color info");
		k_EmptyString(&str);
		return false;
	}
...
/* Same as k_DeriveMomentsGraytone, except that eh weight of each set pixel is one. */
{
	...
	if ((pPixelGroup==NULL) || (pMom==NULL))
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_DeriveMomentsBinary() One of the parameters "
...
/* Calculates central moments from simple moments */
{
	...
	if ((pMom==NULL) || (pCentrMom==NULL))
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_DeriveCentralMoments() One of the "
...
/* Calculates central normalised moments from simple moments */
{
	...
	if ((pMom==NULL) || (pCentrNormMom==NULL))
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_DeriveCentralNormMoments() One of the "
			"parameters are a NULL pointer");
		k_EmptyString(&str);
		return false;
	}
