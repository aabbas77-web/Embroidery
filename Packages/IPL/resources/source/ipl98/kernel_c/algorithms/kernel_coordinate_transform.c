						  ...
						  double phi,UINT32 Background)
{
	...
	if (pSource->Origin==EMPTY)
	{
		...
		k_ShowMessage(IPL_WARNING,&str,"k_ScaleAndRotateAuto() Source image is empty - doing nothing");
		k_EmptyString(&str);
		return false;
	}
	if ((pSource->Bits!=1) && (pSource->Bits!=8) && (pSource->Bits!=24))
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_ScaleAndRotateAuto() Number of bits per pixel=%d not supported",
			pSource->Bits);
		k_EmptyString(&str);
		return false;
	}
	...
	/* add comment to history */
	{
		...
		k_SprintfString(&History,"%sk_ScaleAndRotateAuto(Sx=%g,Sy=%g,phi=%g,Background=%d)",
						   ...
						   int Width,int Height,double Sx, double Sy, double phi, UINT32 Background)
{
	...
	if (pSource->Origin==EMPTY)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_WARNING,&str,"k_ScaleAndRotateFixed() Source image is empty - doing nothing");
		k_EmptyString(&str);
		return false;
	}
	if (pDest==pSource)
	...
	if ((pSource->Bits!=1) && (pSource->Bits!=8) && (pSource->Bits!=24))
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_ScaleAndRotateFixed() Number of bits per pixel=%d not supported",
			pSource->Bits);
		k_EmptyString(&str);
		return false;
	}
	...
	/* add comment to history */
	{
		TString History;
		k_InitString(&History);
		k_SprintfString(&History,"%sk_ScaleAndRotateFixed(x0=%g,y0=%g,Width=%d,Height=%d,Sx=%g,Sy=%g,phi=%g,Background=%d)",
			ipl_HistoryIndent,x0,y0,Width,Height,Sx,Sy,phi,Background);
		k_AppendTextIPL(&(pTarget->History),History.pChars);
		k_EmptyString(&History);
		k_IplStartHistoryMacro();
	}
...
bool k_ScaleAuto(TImage* pSource,TImage* pDest,double Sx, double Sy,UINT32 Background)
{
	...
	if (pSource->Origin==EMPTY)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_WARNING,&str,"k_ScaleAuto() Source image is empty - doing nothing");
		k_EmptyString(&str);
		return false;
	}
	if (pDest==pSource)
	...
	if ((pSource->Bits!=1) && (pSource->Bits!=8) && (pSource->Bits!=24))
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_ScaleAuto() Number of bits per pixel=%d not supported",
			pSource->Bits);
		k_EmptyString(&str);
		return false;
	}
	...
	/* add comment to history */
	{
		TString History;
		k_InitString(&History);
		k_SprintfString(&History,"%sk_ScaleAuto(Sx=%g,Sy=%g,Background=%d)",
				  ...
				  int Width,int Height,double Sx, double Sy,UINT32 Background)
{
	...
	if (pSource->Origin==EMPTY)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_WARNING,&str,"k_ScaleFixed() Source image is empty - doing nothing");
		k_EmptyString(&str);
		return false;
	}
	if (pDest==pSource)
	...
	if ((pSource->Bits!=8) && (pSource->Bits!=24))
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_ScaleFixed() Number of bits per pixel=%d not supported",
			pSource->Bits);
		k_EmptyString(&str);
		return false;
	}
	...
	/* add comment to history */
	{
		TString History;
		k_InitString(&History);
		k_SprintfString(&History,"%sk_ScaleFixed(x0=%g,y0=%g,Width=%d,Height=%d,Sx=%g,Sy=%g,Background=%d)",
