...
bool k_ConvolveByteToByte(TImage *pImg,const TFloatImage *pMask)
{
	...
	if ((W<=3) || (H<=3) || (w>W) || (h>H) || (pImg->Bits!=8))
	{
		...
		if ((W<4) || (H<4))
		{
			k_ShowMessage(IPL_ERROR,&str,"k_ConvolveByteToByte() "
				"Image size %d x %d is too small. Must be at least 4x4",W,H);
		}
		if ((w>W) || (h>H))
		{
			k_ShowMessage(IPL_ERROR,&str,"k_ConvolveByteToByte() "
		...
		if (pImg->Bits!=8)
			k_ShowMessage(IPL_ERROR,&str,"k_ConvolveByteToByte() Image must have 8 b/p");
	...
	else
	{
		...
		/* add comment to history */
		{
			...
			k_SprintfString(&History,"%sk_ConvolveByteToByte() With a %d*%d mask",
						 ...
						 TIntImage *pDest)
{
	...
	if ((W<=3) || (H<=3) || (w>W) || (h>H) || (pSource->Bits!=8))
	{
		...
		k_AddFileAndLine(str);
		if ((W<4) || (H<4))
			k_ShowMessage(IPL_ERROR,&str,"k_ConvolveByteToInt() Image size %d x %d is too small. Must be at least 4x4",W,H);
        if ((w>W) || (h>H))
			k_ShowMessage(IPL_ERROR,&str,"k_ConvolveByteToInt() One or two mask dimensions exceeds the that of the image");
        if (pSource->Bits!=8)
			k_ShowMessage(IPL_ERROR,&str,"k_ConvolveByteToByte() Source image must have 8 b/p");
		k_EmptyString(&str);
		return false;
	}
	else
	{
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%sk_ConvolveByteToInt() With a %d*%d mask",
				ipl_HistoryIndent,w,h);
...
bool k_ConvolveIntToInt(TIntImage *pImg,const TFloatImage *pMask)
{
	...
	if ((W<=3) || (H<=3) || (w>W) || (h>H))
	{
		...
		k_AddFileAndLine(str);
		if ((W<4) || (H<4))
			k_ShowMessage(IPL_ERROR,&str,"k_ConvolveIntToInt() Image size %d x %d is too small. Must be at least 4x4",W,H);
        if ((w>W) || (h>H))
			k_ShowMessage(IPL_ERROR,&str,"k_ConvolveIntToInt() One or two mask dimensions exceeds the that of the image");
		k_EmptyString(&str);
		return false;
	}
	else
	{
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%sk_ConvolveIntToInt() With a %d*%d mask",
				ipl_HistoryIndent,w,h);
			k_AppendTextIPL(&(Temp.History),History.pChars);
			k_EmptyString(&History);
			k_IplStartHistoryMacro();
...
bool k_ConvolveFloatToFloat(TFloatImage *pImg,const TFloatImage *pMask)
{
	...
	if ((W<=3) || (H<=3) || (w>W) || (h>H))
	{
		...
		k_AddFileAndLine(str);
		if ((W<4) || (H<4))
			k_ShowMessage(IPL_ERROR,&str,"k_ConvolveFloatToFloat() Image size %d x %d is too small. Must be at least 4x4",W,H);
        if ((w>W) || (h>H))
			k_ShowMessage(IPL_ERROR,&str,"k_ConvolveFloatToFloat() One or two mask dimensions exceeds the that of the image");
		k_EmptyString(&str);
		return false;
	}
	else
	{
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%sk_ConvolveFloatToFloat() With a %d*%d mask",
				ipl_HistoryIndent,w,h);
			k_AppendTextIPL(&(Temp.History),History.pChars);
			k_EmptyString(&History);
			k_IplStartHistoryMacro();
...
bool k_BlurFourConnected3x3(TImage *pImg)
{
	...
	if ((w<=3) || (h<=3) || (pImg->Bits!=8))
	{
		...
		k_AddFileAndLine(str);
        if (pImg->Bits!=8)
			k_ShowMessage(IPL_ERROR,&str,"k_BlurFourConnected3x3() Image does not have 8 b/p");
        else
			k_ShowMessage(IPL_ERROR,&str,"k_BlurFourConnected3x3() Image size %d x %d is too small. Must be at least 4x4",w,h);
		k_EmptyString(&str);
		return false;
	}
	else
	{
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%sk_BlurFourConnected3x3()",ipl_HistoryIndent);
...
bool k_BlurEightConnected3x3(TImage *pImg)
{
	...
	if ((w<=3) || (h<=3) || (pImg->Bits!=8))
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_BlurEightConnected3x3() Image size %d x %d is too small. Must be at least 4x4",w,h);
		k_EmptyString(&str);
		return false;
	}
	else
	{
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%sk_BlurEightConnected3x3()",ipl_HistoryIndent);
			k_AppendTextIPL(&(pImg->History),History.pChars);
			k_EmptyString(&History);
			k_IplStartHistoryMacro();
		}
...
bool k_BlurEightConnected3x3Int(TIntImage *pImg)
{
	...
	if ((w<=3) || (h<=3) )
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_BlurEightConnected3x3Int() Image size %d x %d is too small. Must be at least 4x4",w,h);
		k_EmptyString(&str);
		return false;
	}
	else
	{
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%sk_BlurEightConnected3x3Int()",ipl_HistoryIndent);
			k_AppendTextIPL(&(pImg->History),History.pChars);
			k_EmptyString(&History);
			k_IplStartHistoryMacro();
		}
...
bool k_ConvolveFastByteToByte(TImage *pImg, const TFloatImage *pMask,float Prefactor)
{
	...
	if ((w!=3) || (h!=3) || (W<=3) || (H<=3) || (pImg->Bits!=8))
	{
		 ...
		 if ((W<4) || (H<4))
		 {
			 k_ShowMessage(IPL_ERROR,&str,"k_ConvolveFastByteToByte() "
				 "Image size %d x %d is too small. Must be at least 4x4",W,H);
		 }
		 else if ((w!=3) || (h!=3))
		 {
			 k_ShowMessage(IPL_ERROR,&str,
				 "k_ConvolveFastByteToByte() Mask size %d x %d is wrong. Mask must be 3x3",w,h);
		 }
		 else
			 k_ShowMessage(IPL_ERROR,&str,"k_ConvolveFastByteToByte() Image does not have 8 b/p");
	...
	else
	{
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%sk_ConvolveFastByteToByte() prefactor=%g",
							 ...
							 float Prefactor, TIntImage *pDest)
{
	...
	if ((w!=3) || (h!=3) || (W<=3) || (H<=3) || (pSource->Bits!=8))
	{
	   TString str;
		...
		{
			k_ShowMessage(IPL_ERROR,&str,
				"k_ConvolveFastByteToInt() Image size %d x %d is too small. Must be at least 4x4",W,H);
		}
		else
		{
			if ((w!=3) || (h!=3)) 
				k_ShowMessage(IPL_ERROR,&str,"k_ConvolveFastByteToInt() Mask size %d x %d is wrong. Mask must be 3x3",w,h);
			else
				k_ShowMessage(IPL_ERROR,&str,"k_ConvolveFastByteToInt() Source image does not have 8 b/p"); 
		}
		k_EmptyString(&str);
		return false;
	}
	else
	{
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%sk_ConvolveFastByteToInt() prefactor=%g",
				ipl_HistoryIndent,Prefactor);
			k_AppendTextIPL(&(pDest->History),History.pChars);
			k_EmptyString(&History);
			k_IplStartHistoryMacro();
...
bool k_ConvolveFastIntToInt(TIntImage *pImg, const TFloatImage *pMask,float Prefactor)
{
	...
	if ((w!=3) || (h!=3) || (W<=3) || (H<=3))
	{
	   TString str;
		...
		{
			k_ShowMessage(IPL_ERROR,&str,
				"k_ConvolveFastIntToInt() Image size %d x %d is too small. Must be at least 4x4",W,H);
		}
		if ((w!=3) || (h!=3)) 
		{
			k_ShowMessage(IPL_ERROR,&str,
				"k_ConvolveFastIntToInt() Mask size %d x %d is wrong. Mask must be 3x3",w,h);
		}
		k_EmptyString(&str);
		return false;
	}
	else
	{
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%sk_ConvolveFastIntToInt() prefactor=%g",
				ipl_HistoryIndent,Prefactor);
			k_AppendTextIPL(&(Temp.History),History.pChars);
			k_EmptyString(&History);
			k_IplStartHistoryMacro();
...
bool k_MaxGradientFastByteToByte(TImage *pImg,float Prefactor)
{
	...
	if ((W<=3) || (H<=3) || (pImg->Bits!=8))
	{
        ...
        if (pImg->Bits!=8)
		{
			k_ShowMessage(IPL_ERROR,&str,"k_MaxGratientFastByteToByte() Image does not have 8 Bits/pixel");
		}
		else
		{
			k_ShowMessage(IPL_ERROR,&str,
				"k_MaxGratientFastByteToByte() Image size %d x %d is too small. Must be at least 4x4",W,H);
		}
		k_EmptyString(&str);
		return false;
	}
	else
	{
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%sk_MaxGradientFastByteToByte() prefactor=%g",
				ipl_HistoryIndent,Prefactor);
			k_AppendTextIPL(&(Temp.History),History.pChars);
			k_EmptyString(&History);
			k_IplStartHistoryMacro();
...
bool k_MaxGradientFastByteToInt(TImage *pSource,float Prefactor,TIntImage *pDest)
{
	...
	if ((W<=3) || (H<=3) || (pSource->Bits!=8))
	{
		...
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,
			"k_MaxGratientFastByteToInt() Image size %d x %d is too small. Must be at least 4x4",W,H);
		k_EmptyString(&str);
		return false;
	}
	else
	{
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%sk_MaxGradientFastByteToInt() prefactor=%g",
				ipl_HistoryIndent,Prefactor);
			k_AppendTextIPL(&(pDest->History),History.pChars);
			k_EmptyString(&History);
			k_IplStartHistoryMacro();
...
bool k_MaxGradientFastIntToInt(TIntImage *pImg,float Prefactor)
{
	...
	if ((W<=3) || (H<=3))
	{
		...
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,
			"k_MaxGratientFastIntToInt(). Image size %d x %d is too small. Must be at least 4x4",W,H);
		k_EmptyString(&str);
		return false;
	}
	else
	{
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%sk_MaxGradientFastIntToInt() prefactor=%g",
				ipl_HistoryIndent,Prefactor);
			k_AppendTextIPL(&(Temp.History),History.pChars);
			k_EmptyString(&History);
			k_IplStartHistoryMacro();
...
bool k_MedianFilterFourConnected3x3(TImage *pImg)
{
	...
	if ((W<=3) || (H<=3) || (pImg->Bits!=8))
	{
		...
		k_AddFileAndLine(str);
        if (pImg->Bits!=8)
			k_ShowMessage(IPL_ERROR,&str,"k_MedianFilterFourConnected3x3() Image does not have 8 Bits/pixel"); 
		else
			k_ShowMessage(IPL_ERROR,&str,"k_MedianFilterFourConnected3x3() Image size %d x %d is too small. Must be at least 4x4",W,H);
		k_EmptyString(&str);
		return false;
	}
	else
	{
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%sk_MedianFilterFourConnected3x3()",ipl_HistoryIndent);
			k_AppendTextIPL(&(Temp.History),History.pChars);
			k_EmptyString(&History);
			k_IplStartHistoryMacro();
		}
...
bool k_MedianFilterDiagonalConnected3x3(TImage *pImg)
{
	...
	if ((W<=3) || (H<=3) || (pImg->Bits!=8))
	{
		...
		k_AddFileAndLine(str);
		if (pImg->Bits!=8)
			k_ShowMessage(IPL_ERROR,&str,"k_MedianFilterDiagonalConnected3x3() Image does not have 8 b/p"); 
		else
			k_ShowMessage(IPL_ERROR,&str,"k_MedianFilterDiagonalConnected3x3() Image size %d x %d is too small. Must be at least 4x4",W,H);
		k_EmptyString(&str);
		return false;
	}
	else
	{
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%sk_MedianFilterDiagonalConnected3x3()",ipl_HistoryIndent);
			k_AppendTextIPL(&(Temp.History),History.pChars);
			k_EmptyString(&History);
			k_IplStartHistoryMacro();
		}
...
bool k_MedianFilterFourConnected3x3Int(TIntImage *pImg)
{
	...
	if ((W<=3) || (H<=3))
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_MedianFilterFourConnected3x3Int() Image size %d x %d is too small. Must be at least 4x4",W,H);
		k_EmptyString(&str);
		return false;
	}
	else
	{
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%sk_MedianFilterFourConnected3x3Int()",ipl_HistoryIndent);
			k_AppendTextIPL(&(Temp.History),History.pChars);
			k_EmptyString(&History);
			k_IplStartHistoryMacro();
		}
...
bool k_MedianFilterDiagonalConnected3x3Int(TIntImage *pImg)
{
	...
	if ((W<=3) || (H<=3))
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_MedianFilterDiagonalConnected3x3Int(). Image size %d x %d is too small. Must be at least 4x4",W,H);
		k_EmptyString(&str);
		return false;
	}
	else
	{
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%sk_MedianFilterDiagonalConnected3x3Int()",ipl_HistoryIndent);
			k_AppendTextIPL(&(Temp.History),History.pChars);
			k_EmptyString(&History);
			k_IplStartHistoryMacro();
		}
