...
void k_ReAllocString(TString* pString,unsigned int NewSize)
{
	...
	else
	{
		...
		k_ShowMessage(IPL_ERROR,&str,"k_ReAllocString() Realloc failed");
	    k_EmptyString(&str);
	}
}
...
void k_SprintfString(TString* pString,char* fmt, ...)
{
	...
	if ((unsigned int)vsprintf(p,fmt,args)>=MAX_STRING_SIZE)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_SprintfString() array contained in the parameter fmt too long");
		k_EmptyString(&str);
		return;
	}
	else
...
bool k_InsertCharInString(char Character, unsigned int Index, TString* pString)
{
	if (Index>=pString->NumberOfChars)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_InsertCharInString() Index (%d) is out of range (NumberOfChars=%d)",
...
bool k_RemoveCharFromString(unsigned int Index, TString* pString)
{
	if (Index>=pString->NumberOfChars)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_RemoveCharFromString() Index (%d) is out of range (NumberOfChars=%d)",
			Index,pString->NumberOfChars);
	    k_EmptyString(&str);
		return false;
	}
