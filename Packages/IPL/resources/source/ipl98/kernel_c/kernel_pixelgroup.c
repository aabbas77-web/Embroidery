...
void k_ReAllocGroup(TPixelGroup* pPixelGroup,unsigned int NewSize)
{
	if ((pPixelGroup->pPos=(T2DInt*)realloc(pPixelGroup->pPos,NewSize*sizeof(T2DInt)))!=NULL)
	{
		...
		if (pPixelGroup->pColor!=NULL)
		{
			if ((pPixelGroup->pColor=(UINT32*)realloc(pPixelGroup->pColor,NewSize*sizeof(UINT32)))!=NULL)
			{
				...
				k_ShowMessage(IPL_ERROR,&str,"k_ReAllocGroup() Realloc colors failed");
				k_EmptyString(&str);
			}
		}
	}
	else
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_ReAllocGroup() Realloc of positions failed");
		k_EmptyString(&str);
	}
}
...
bool k_AddPosToGroup(INT16 x, INT16 y, TPixelGroup* pPixelGroup)
{
	if (pPixelGroup->pColor!=NULL)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_AddPosToGroup() Colors available, cannot insert position only");
	    k_EmptyString(&str);
		return false;
	}
	else
...
bool k_Add2DPosToGroup(T2DInt Pos, TPixelGroup* pPixelGroup)
{
	if (pPixelGroup->pColor!=NULL)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_Add2DPosToGroup() Colors available, cannot insert position only");
	    k_EmptyString(&str);
		return false;
	}
	else
...
bool k_AddPosColorToGroup(INT16 x, INT16 y, UINT32 Color, TPixelGroup* pPixelGroup)
{
	if (pPixelGroup->pColor==NULL)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_AddPosColorToGroup() Colors not available");
	    k_EmptyString(&str);
		return false;
	}
	else
...
bool k_Add2DPosColorToGroup(T2DInt Pos, UINT32 Color, TPixelGroup* pPixelGroup)
{
	if (pPixelGroup->pColor==NULL)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_Add2DPosColorToGroup() Colors not available");
	    k_EmptyString(&str);
		return false;
	}
	else
...
{	/* C only version, C++ has its own implementation to increase speed */
	...
	else
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_GetColorGroup() Index(%d) out of range or colors not available",Index);
	    k_EmptyString(&str);
		return 0;
	}
}
...
{	/* C only version, C++ has its own implementation to increase speed */
	...
	else
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_SetColorGroup() Index(%d) out of range or colors not available",Index);
	    k_EmptyString(&str);
		return false;
	}
}
...
bool k_InsertPosInGroup(INT16 x, INT16 y, unsigned int Index, TPixelGroup* pPixelGroup)
{
	if (Index>=pPixelGroup->NumberOfPixels)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_InsertPosInGroup() Index (%d) is out of range (NumberOfPixels=%d)",
	...
	if (pPixelGroup->pColor!=NULL)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_InsertPosInGroup() Colors are present - failed inserting");
		k_EmptyString(&str);
		return false;
	}
...
bool k_Insert2DPosInGroup(T2DInt Pos, unsigned int Index, TPixelGroup* pPixelGroup)
{
	if (Index>=pPixelGroup->NumberOfPixels)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_Insert2DPosInGroup() Index (%d) is out of range (NumberOfPixels=%d)",
			Index,pPixelGroup->NumberOfPixels);
		k_EmptyString(&str);
		return false;
	}
...
bool k_InsertPosColorInGroup(INT16 x, INT16 y, UINT32 Color, unsigned int Index, TPixelGroup* pPixelGroup)
{
	if (Index>=pPixelGroup->NumberOfPixels)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_InsertPosColorInGroup() Index (%d) is out of range (NumberOfPixels=%d)",
			Index,pPixelGroup->NumberOfPixels);
		k_EmptyString(&str);
		return false;
	}
	if (pPixelGroup->pColor==NULL)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_InsertPosInGroup() Colors not available - failed inserting");
		k_EmptyString(&str);
		return false;
	}
	/* Add a dummy position to the end of position array */
...
bool k_Insert2DPosColorInGroup(T2DInt Pos, UINT32 Color, unsigned int Index, TPixelGroup* pPixelGroup)
{
	if (Index>=pPixelGroup->NumberOfPixels)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_Insert2DPosColorInGroup() Index (%d) is out of range (NumberOfPixels=%d)",
			Index,pPixelGroup->NumberOfPixels);
		k_EmptyString(&str);
		return false;
	}
	if (pPixelGroup->pColor==NULL)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_Insert2DPosInGroup() Colors not available - failed inserting");
		k_EmptyString(&str);
		return false;
	}
	/* Add a dummy position to the end of position array */
...
bool k_RemovePosFromGroup(unsigned int Index, TPixelGroup* pPixelGroup)
{
	...
	if (Index>=pPixelGroup->NumberOfPixels)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_RemovePosFromGroup() Index (%d) is out of range (NumberOfPixels=%d)",
			Index,pPixelGroup->NumberOfPixels);
		k_EmptyString(&str);
		return false;
	}
...
bool k_RemovePosFromGroupSlow(unsigned int Index, TPixelGroup* pPixelGroup)
{
	...
	if (Index>=pPixelGroup->NumberOfPixels)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_RemovePosFromGroupSlow() Index (%d) is out of range (NumberOfPixels=%d)",
			Index,pPixelGroup->NumberOfPixels);
		k_EmptyString(&str);
		return false;
	}
...
bool k_AllocColorsGroup(TPixelGroup* pPixelGroup)
{
	...
	else
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_AllocColorsGroup() PixelGroup already contains colors");
		k_EmptyString(&str);
		return false;
	}
}
...
bool k_AllocAndInitColorsGroup(UINT32 InitialValue,TPixelGroup* pPixelGroup)
{
	...
	else
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_AllocAndInitColorsGroup() PixelGroup already contains colors");
		k_EmptyString(&str);
		return false;
	}
}
...
bool k_AddColorsToGroup(const TImage* pSource,TPixelGroup* pPixelGroup)
{
	...
	if ((pPixelGroup==NULL) || (pSource==NULL))
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_AddColorsToGroup() At least one of the parameters are NULL");
		k_EmptyString(&str);
		return false;
	}
	if(pPixelGroup->NumberOfPixels==0)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_WARNING,&str,"k_AddColorsToGroup() PixelGroup contains no pixels - doing nothing");
		k_EmptyString(&str);
		return false;
	}
		...
		((UINT32)(pPixelGroup->Right.x-pPixelGroup->Left.x+1)>pSource->Width))
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_AddColorsToGroup() Dimensions in PixelGroup (%d*%d) bigger than source image (%d*%d)",
			(pPixelGroup->Bottom.y-pPixelGroup->Top.y+1),(pPixelGroup->Right.x-pPixelGroup->Left.x+1),pSource->Width,pSource->Height);
		k_EmptyString(&str);
		return false;
	}
	if (pPixelGroup->pColor!=NULL)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_AddColorsToGroup() PixelGroup already contains colors");
		k_EmptyString(&str);
		return false;
	}
...
bool k_CopyGroupToImage(TPalette* pPalette, UINT32 BackGround, TImage* pDest, TPixelGroup* pPixelGroup)
{
	unsigned int i;
	T2DInt r;
	if ((pDest==NULL) || (pPixelGroup==NULL))
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_CopyGroupToImage() pPixelGroup or pDest parameter is NULL");
		k_EmptyString(&str);
		return false;
	}
	...
	if (pPixelGroup->NumberOfPixels==0)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_CopyGroupToImage() no points stored in pPixelGroup");
		k_EmptyString(&str);
		return false;
	}
	...
	if (pPixelGroup->pColor==NULL)
	{
		if (pPalette!=NULL)
		{
			...
			k_InitString(&str);
			k_AddFileAndLine(str);
			k_ShowMessage(IPL_WARNING,&str,"k_CopyGroupToImage() No colors available, ignoring the palette provided");
			k_EmptyString(&str);
		}
	...
	else /* colors and palette are avialable - no check */
	{
		if ((pPixelGroup->Bits==8) && (pPalette->PalEntries!=256))
		{
			...
			k_InitString(&str);
			k_AddFileAndLine(str);
			k_ShowMessage(IPL_ERROR,&str,"k_CopyGroupToImage() pPixelGroup pixel depth is %d but pPalette has %d entries, should be 256",
		...
		else if ((pPixelGroup->Bits==1) && (pPalette->PalEntries!=2))
		{
			...
			k_InitString(&str);
			k_AddFileAndLine(str);
			k_ShowMessage(IPL_ERROR,&str,"k_CopyGroupToImage() pPixelGroup pixel depth is %d but pPalette has %d entries, should be 2",
				pPixelGroup->Bits,pPalette->PalEntries);
			k_EmptyString(&str);
			return false;
		}
		else
		{
			...
			if (pPixelGroup->Bits==24)
			{
				...
				k_InitString(&str);
				k_AddFileAndLine(str);
				k_ShowMessage(IPL_WARNING,&str,"k_CopyGroupToImage() pPixelGroup bit depth is 24 b/p, ignoring the palette provided");
				k_EmptyString(&str);
			}
			else
...
bool k_AddGroupToImage(TImage* pDest, const TPixelGroup* pPixelGroup,UINT32 Color)
{
	...
	if (pPixelGroup==NULL)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_AddGroupToImage() pPixelGroup is NULL");
		k_EmptyString(&str);
		return false;
	}
	/* add positions */
		...
		default:
			{
				...
				k_InitString(&str);
				k_AddFileAndLine(str);
				k_ShowMessage(IPL_ERROR,&str,"k_AddGroupToImage() Only implemented for 1,8 and 24 bit per pixels");
				k_EmptyString(&str);
				return false;
			}
		}
...
bool k_AddGroupToComplexImage(TComplexImage* pDest, const TPixelGroup* pPixelGroup, FLOAT32 Value)
{
	...
	if (pPixelGroup==NULL)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_AddGroupToComplexImage() pPixelGroup is NULL");
		k_EmptyString(&str);
		return false;
	}
	/* add positions */
...
bool k_AddGroupToFloatImage(TFloatImage* pDest, const TPixelGroup* pPixelGroup, FLOAT32 Value)
{
	...
	if (pPixelGroup==NULL)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_AddGroupToFloatImage() pPixelGroup is NULL");
		k_EmptyString(&str);
		return false;
	}
	/* add positions */
...
bool k_AddGroupToIntImage(TIntImage* pDest, const TPixelGroup* pPixelGroup, INT16 Value)
{
	...
	if (pPixelGroup==NULL)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_AddGroupToIntImage() pPixelGroup is NULL");
		k_EmptyString(&str);
		return false;
	}
	/* add positions */
...
bool k_AddGroupColorsToImage(TImage* pDest, const TPixelGroup* pPixelGroup)
{
	...
	if (pPixelGroup==NULL)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_AddGroupColorsToImage() pPixelGroup is NULL");
		k_EmptyString(&str);
		return false;
	}
	else if (pPixelGroup->pColor==NULL)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_AddGroupColorsToImage() No colors in PixelGroup");
		k_EmptyString(&str);
		return false;
	}
	/* add positions */
		...
		default:
			{
				...
				k_InitString(&str);
				k_AddFileAndLine(str);
				k_ShowMessage(IPL_ERROR,&str,"k_AddGroupColorsToImage() Only implemented for 1,8 and 24 bit per pixels");
				k_EmptyString(&str);
				return false;
			}
		}
			...
			default:
				{
					...
					k_InitString(&str);
					k_AddFileAndLine(str);
					k_ShowMessage(IPL_ERROR,&str,"k_AddGroupToImageFast() Only implemented for 1,8 and 24 bit per pixels");
					k_EmptyString(&str);
					return false;
				}
			}
...
bool k_GroupToBinaryImage(const TPixelGroup* pPixelGroup,COLORTYPE Color,TImage* pDest)
{
	...
	T2DInt r;
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_WARNING,&str,"k_GroupToBinaryImage() This function will be removed in next version - use k_CopyGroupToImage()");
		k_EmptyString(&str);
	}
	if (pPixelGroup==NULL)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_GroupToBinaryImage() pPixelGroup parameter is NULL");
		k_EmptyString(&str);
		return false;
	}
						   ...
						   const TImage* pSource, TImage* pDest)
{
	...
	T2DInt r;
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_WARNING,&str,"k_GroupToGrayImage() This function will be removed in next version - use k_CopyGroupToImage()");
		k_EmptyString(&str);
	}
	if ((pPixelGroup==NULL) || (pSource==NULL) || (pDest==NULL))
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_GroupToGrayImage() One of the parameters is NULL");
	    k_EmptyString(&str);
		return false;
	}
							  ...
							  TImage* pDest)
{
	...
	T2DInt r;
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_WARNING,&str,"k_GroupColorsToGrayImage() This function will be removed in next version - use k_CopyGroupToImage()");
		k_EmptyString(&str);
	}
...
bool k_PosInGroup(const TPixelGroup* pPixelGroup, T2DInt Pos,unsigned int* pIndex)
{
	if (pPixelGroup==NULL)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_PosInGroup() pPixelGroup parameter is NULL");
	    k_EmptyString(&str);
		return false;
	}
	else
