...
/** @defgroup image Class Image
	Kernel ANSI C image routines, last updated 7/15/2002.
...
/** Macro function for setting pixels in an image with 24 b/p only. The macro is very
	...
	@param Image (TImage) The image to set pixel in. */
#ifdef IPL_BIG_ENDIAN
#define k_SetPixel24bp(x,y,Color,Image) {(Image).ppMatrix[(y)][3*(x)]=(UINT8)Color;(Image).ppMatrix[(y)][3*(x)+1]=(UINT8)(Color>>8);(Image).ppMatrix[(y)][3*(x)+2]=(UINT8)(Color>>16);}
#else
