...
bool kF_AllocImage(UINT32 Width, UINT32 Height,TFloatImage* pInfo)
{
	...
	else
	{
		...
		k_ShowMessage(IPL_ERROR,&str,"kF_AllocImage() pInfo is a NULL pointer");
		k_EmptyString(&str);
		return false;
	}
}
...
bool kF_AllocImageFast(UINT32 Width, UINT32 Height,TFloatImage* pInfo)
{
	...
	else
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kF_AllocImageFast() pInfo is a NULL pointer");
		k_EmptyString(&str);
		return false;
	}
}
...
bool kF_FlushImage(FLOAT32 Color,TFloatImage* pDest)
{
	...
	if (pDest==NULL)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kF_FlushImage() pDest is a NULL pointer");
		k_EmptyString(&str);
		return false;
	}
	else if (pDest->Origin==EMPTY)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_WARNING,&str,"kF_FlushImage() Image is empty - doing nothing");
		k_EmptyString(&str);
		return false;
	}
	else
	{
		...
		/* add comment to history */
		{
			...
			k_SprintfString(&History,"%skF_FlushImage(Color=%d)",ipl_HistoryIndent,Color);
...
bool kF_Load(const char* FileName, TFloatImage* pInfo)
{
	...
	if (!k_SplitFileName(FileName, &Path,&FileNameExt,&Ext))
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kF_Load() Filename not valid");
		k_EmptyString(&str);
		return false;
	}
	else if (strcmp(Ext,"")==0)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kF_Load() No extension found");
		k_EmptyString(&str);
		returnValue=false;
	}
	else
	{
		...
		if (fp==NULL)
		{
			...
			k_InitString(&str);
			k_AddFileAndLine(str);
			k_ShowMessage(IPL_ERROR,&str,"kF_Load() Could not open file");
			k_EmptyString(&str);
			returnValue=false;
		}
		else
		...
		if (returnValue==true)
		{
			...
			/* add comment to history */
			{
				TString History;
				k_InitString(&History);
				k_SprintfString(&History,"%skF_Load(%s)",ipl_HistoryIndent,FileName);
...
bool kF_Save(const char* FileName, TFloatImage* pInfo)
{
	...
	if (!k_SplitFileName(FileName, &Path,&FileNameExt,&Ext))
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kF_Save() Filename not valid");
		k_EmptyString(&str);
		return false;
	}
	else if (strcmp(Ext,"")==0)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kF_Save() No extension found");
		k_EmptyString(&str);
		returnValue=false;
	}
	else
	{
		...
		if (fp==NULL)
		{
			...
			k_InitString(&str);
			k_AddFileAndLine(str);
			k_ShowMessage(IPL_ERROR,&str,"kF_Save() Could not save file");
			k_EmptyString(&str);
			returnValue=false;
		}
		else if (pInfo->Origin==EMPTY)
		{
			...
			k_InitString(&str);
			k_AddFileAndLine(str);
			k_ShowMessage(IPL_ERROR,&str,"kF_Save() No image - cannot save");
			k_EmptyString(&str);
			returnValue=false;
		}
		else
		{
			...
			/* add comment to history */
			{
				TString History;
				k_InitString(&History);
				k_SprintfString(&History,"%skF_Save(%s)",ipl_HistoryIndent,FileName);
				k_AppendTextIPL(&(pInfo->History),History.pChars);
				k_EmptyString(&History);
			}
#endif
...
bool kF_SetPeriodicBorder(unsigned short BorderSize,TFloatImage* pInfo)
{
	if (pInfo->Origin==EMPTY)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kF_SetPeriodicBorder() Image is empty");
		k_EmptyString(&str);
		return false;
	}
	else if (((unsigned int)BorderSize>pInfo->Width) || ((unsigned int)BorderSize>pInfo->Height))
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kF_SetPeriodicBorder() Border too large");
		k_EmptyString(&str);
		return false;
	}
	else
...
bool kF_CopyImage(TFloatImage* pDest,const TFloatImage* pSource)
{
	if (pDest==pSource)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kF_CopyImage() Source and destination image"
	...
	if (pSource->Origin!=EMPTY)
	{
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%skF_CopyImage() Source image file info: %s"
				  ...
				  TFloatImage* pDest,const TFloatImage* pSource)
{
	...
	if (pSource->Origin==EMPTY)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_WARNING,&str,"kF_CopySubImage() Source image is empty - doing nothing");
		k_EmptyString(&str);
		return false;
	}
		...
		(xmax<=kF_GetMaxX(pSource)) && (ymax<=kF_GetMaxY(pSource)))
	{
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%skF_CopySubImage(xmin=%d,ymin=%d,xmax=%d,ymax=%d) Source image file info: %s"
	...
	else
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kF_CopySubImage() Illegal clipping rectangel");
		k_EmptyString(&str);
		return false;
	}
}
...
void kF_Rotate90(int Steps,TFloatImage* pSource)
{
	...
	if (pSource->Origin==EMPTY)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_WARNING,&str,"kF_Rotate90() Image is empty - doing nothing");
		k_EmptyString(&str);
		return;
	}
	if (ActSteps==0)
	...
	/* add comment to history */
	{
		TString History;
		k_InitString(&History);
		k_SprintfString(&History,"%skF_Rotate90(Steps=%d)",ipl_HistoryIndent,Steps);
...
void kF_FlipVertical(TFloatImage* pSource)
{
	...
	if (pSource->Origin==EMPTY)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_WARNING,&str,"kF_FlipVertical() Image is empty - doing nothing");
		k_EmptyString(&str);
		return;
	}
	...
	/* add comment to history */
	{
		TString History;
		k_InitString(&History);
		k_SprintfString(&History,"%skF_FlipVertical()",ipl_HistoryIndent);
		k_AppendTextIPL(&(NewImage.History),History.pChars);
		k_EmptyString(&History);
		k_IplStartHistoryMacro();
	}
...
bool kF_ApplyBias(float Bias,TFloatImage *pImg)
{
	if (pImg->Origin!=EMPTY)
	{
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%skF_ApplyBias() Bias=%g",ipl_HistoryIndent,Bias);
	...
	else
	{
        ...
        k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kF_ApplyBias: Source image empty");
		k_EmptyString(&str);
		return false;
	}
}
...
bool kF_ScaleIntensity(float Prefactor, TFloatImage *pImg)
{
	if (pImg->Origin!=EMPTY)
	{
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%skF_ScaleIntensity() Prefactor=%g",ipl_HistoryIndent,Prefactor);
			k_AppendTextIPL(&(pImg->History),History.pChars);
			k_EmptyString(&History);
			k_IplStartHistoryMacro();
		}
	...
	else
	{
        ...
        k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kF_ScaleIntensity: Source image empty");
		k_EmptyString(&str);
		return false;
	}
}
...
bool kF_Add( TFloatImage *pImg,TFloatImage *pAddend)
{
		...
		|| (pImg->Width!=pAddend->Width) || (pImg->Height!=pAddend->Height))
	{
        ...
        k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kF_Add(): Images are empty or of different size");
		k_EmptyString(&str);
		return false;
	}
	else
	{
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%skF_Add() Adding a TIFloatmage",ipl_HistoryIndent);
			k_AppendTextIPL(&(pImg->History),History.pChars);
			k_EmptyString(&History);
			k_IplStartHistoryMacro();
		}
...
bool kF_Subtract(TFloatImage *pImg,TFloatImage *pSubtrahend)
{
		...
		|| (pImg->Width!=pSubtrahend->Width) || (pImg->Height!=pSubtrahend->Height))
	{
        ...
        k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kF_Subtract(): Images are empty or of different size");
		k_EmptyString(&str);
		return false;
	}
	else
	{
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%skF_Subtract() subtracting a TFloatImage",ipl_HistoryIndent);
			k_AppendTextIPL(&(pImg->History),History.pChars);
			k_EmptyString(&History);
			k_IplStartHistoryMacro();
		}
...
bool kF_Multiply( TFloatImage *pImg,TFloatImage *pFactor)
{
		...
		|| (pImg->Width!=pFactor->Width) || (pImg->Height!=pFactor->Height))
	{
        ...
        k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kF_Multiply(): Images are empty or of different size");
		k_EmptyString(&str);
		return false;
	}
	else
	{
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%skF_Multiply() multiplying a TFloatImage",ipl_HistoryIndent);
			k_AppendTextIPL(&(pImg->History),History.pChars);
			k_EmptyString(&History);
			k_IplStartHistoryMacro();
		}
...
bool kF_CopyDisplace(int dx,int dy, TFloatImage *pImg)
{
	if (pImg->Origin!=EMPTY)
	{
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%skF_CopyDisplace() dx=%d dy=%d",ipl_HistoryIndent,dx,dy);
	...
	else
	{
        ...
        k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kF_CopyDisplace(): ComplexImage is empty");
		k_EmptyString(&str);
		return false;
	}
}
...
bool kF_CutOrZeroPad(TFloatImage*pDest,TFloatImage *pSource)
{
		...
		&& (ws>0) && (hs>0) && (wd>0) && (hd>0) && (pSource!=pDest))
	{
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%skF_CutOrZeroPad()",ipl_HistoryIndent);
			k_AppendTextIPL(&(pDest->History),History.pChars);
			k_EmptyString(&History);
			k_IplStartHistoryMacro();
		}
	...
	else
	{
		...
		if (pSource==pDest)
		{
			k_ShowMessage(IPL_ERROR,&str,"kF_CutOrZeroPad(): Cannot use same image as dest/source"); 
		}
		else if ((ws<=0) || (hs<=0) || (wd<=0) || (hd<=0))
		{
			k_ShowMessage(IPL_ERROR,&str,"kF_CutOrZeroPad(): One or both images have no pixels"); 
		}
		else
		{
			k_ShowMessage(IPL_ERROR,&str,"kF_CutOrZeroPad(): One or both images are empty");
		}
		k_EmptyString(&str);
		return false;
	}
...
bool kF_CopyFromByte(TFloatImage *pDest,TImage *pSource)
{
	if (pSource->Origin==EMPTY)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kF_CopyFromByte(): Image is empty");
		k_EmptyString(&str);
		k_EmptyString(&str);
		return false;
	}
	else
	{
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%skF_CopyFromByte()",ipl_HistoryIndent);
			k_AppendTextIPL(&(pDest->History),History.pChars);
			k_EmptyString(&History);
			k_IplStartHistoryMacro();
		}
...
bool kF_CopyFromInt(TFloatImage *pDest,TIntImage *pSource)
{
	if (pSource->Origin==EMPTY)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kF_CopyFromInt(): Image is empty");
		k_EmptyString(&str);
		k_EmptyString(&str);
		return false;
	}
	else
	{
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%skF_CopyFromInt()",ipl_HistoryIndent);
			k_AppendTextIPL(&(pDest->History),History.pChars);
			k_EmptyString(&History);
			k_IplStartHistoryMacro();
		}
...
bool kF_CopyFromComplex(COMPLEXCHOISE mode, TFloatImage *pDest,TComplexImage *pSource)
{
	if (pSource->Origin==EMPTY)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kF_CopyFromComplex(): Image is empty");
		k_EmptyString(&str);
		k_EmptyString(&str);
		return false;
	}
	else
	{
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%skF_CopyFromComplex()",ipl_HistoryIndent);
			k_AppendTextIPL(&(pDest->History),History.pChars);
			k_EmptyString(&History);
			k_IplStartHistoryMacro();
		}
...
bool kF_DrawLine(T2DInt Start, T2DInt End, FLOAT32 Value, TFloatImage *pDest)
{
	...
	if (pDest==NULL)
	{
        ...
        k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kF_DrawLine(): Image is empty");
		k_EmptyString(&str);
		return false;
	}
	k_InitGroup(&grp);
...
bool kF_DrawCircle(T2DInt Center, unsigned int Radius, FLOAT32 Value, TFloatImage *pDest)
{
	...
	if (pDest==NULL)
	{
        ...
        k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kF_DrawCircle(): Image is empty");
		k_EmptyString(&str);
		return false;
	}
	k_InitGroup(&grp);
...
FLOAT32 kF_GetPixel(int x, int y,const TFloatImage* pInfo)
{
	...
	else
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kF_GetPixel(%d,%d) Outside image and border",x,y);
		k_EmptyString(&str);
		return 0;
	}
}
...
bool kF_SetPixel(int x, int y, FLOAT32 Color,TFloatImage* pInfo)
{
	...
	else
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kF_SetPixel(%d,%d) Outside image and border",x,y);
		k_EmptyString(&str);
		return false;
	}
}
...
FLOAT32 kF_GetPixelInterpolated(float x, float y, const TFloatImage* pInfo)
{
	...
	else
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kF_GetPixelInterpolated(%.2f,%.2f) Needs "
...
bool kF_LoadFloat(FILE* fp,TFloatImage* pInfo)
{
	...
	while (fgetc(fp)=='#')
	{
		if ((fgetc(fp)!='#') && (TokenRead==false))
		{
			...
			if (CharsInLine==-1)
			{
				...
				k_InitString(&str);
				k_AddFileAndLine(str);
				k_ShowMessage(IPL_ERROR,&str,"kF_LoadFloat() wrong header - "
		...
		else
		{
			...
			if (Token==NULL)
			{
				...
				k_InitString(&str);
				k_AddFileAndLine(str);
				k_ShowMessage(IPL_ERROR,&str,"kF_LoadFloat() wrong file format, end of file reached");
				k_EmptyString(&str);
				return false;
			}
			else if (strcmp(Token,"WidthHeight")==0)
			{
				...
				if (WidthHeightRead==true)
				{
					...
					if ((Width!=Cols) || (Height!=Rows))
					{
						...
						k_InitString(&str);
						k_AddFileAndLine(str);
						k_ShowMessage(IPL_ERROR,&str,"kF_LoadFloat() Width=Cols (%d=%d) and height=Rows (%d,%d) "
	...
	if (WidthHeightRead==false)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kF_LoadFloat() wrong header - width and height not found");
		k_EmptyString(&str);
		return false;
	}
	/* allocate image */
		...
		for(x=0;x<Width;x++)
		{
			if (k_SkipWhiteSpace(fp)!=-1)
			{
				...
				else
				{
					...
					k_InitString(&str);
					k_AddFileAndLine(str);
					k_ShowMessage(IPL_ERROR,&str,"kF_LoadFloat() Failed reading image data");
					k_EmptyString(&str);
					kF_EmptyImage(pInfo);
					return false;
				}
...
bool kF_SaveFloat(FILE* fp,TFloatImage* pInfo)
{
	...
	if (pInfo->Origin==EMPTY)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_WARNING,&str,"kF_SaveFloat() No image to save");
		k_EmptyString(&str);
		fclose(fp);
		return false;
	}
