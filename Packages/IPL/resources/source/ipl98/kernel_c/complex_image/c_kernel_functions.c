...
bool kC_AllocImage(UINT32 Width, UINT32 Height,TComplexImage* pInfo)
{
	...
	else
	{
		...
		k_ShowMessage(IPL_ERROR,&str,"kC_AllocImage() pInfo is a NULL pointer");
		k_EmptyString(&str);
		return false;
	}
}
...
bool kC_AllocImageFast(UINT32 Width, UINT32 Height,TComplexImage* pInfo)
{
	...
	else
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kC_AllocImageFast() pInfo is a NULL pointer");
		k_EmptyString(&str);
		return false;
	}
}
...
bool kC_FlushImage(FLOAT32 Re, FLOAT32 Im,TComplexImage* pDest)
{
	...
	if (pDest==NULL)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kC_FlushImage() pDest is a NULL pointer");
		k_EmptyString(&str);
		return false;
	}
	else if (pDest->Origin==EMPTY)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_WARNING,&str,"kC_FlushImage() Image is empty - doing nothing");
		k_EmptyString(&str);
		return false;
	}
	else
	{
		...
		/* add comment to history */
		{
			...
			k_SprintfString(&History,"%skC_FlushImage(Re=%f Im=%f)",ipl_HistoryIndent,Re,Im);
...
bool kC_Load(const char* FileName, TComplexImage* pInfo)
{
	...
	if (!k_SplitFileName(FileName, &Path,&FileNameExt,&Ext))
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kC_Load() Filename not valid");
		k_EmptyString(&str);
		return false;
	}
	else if (strcmp(Ext,"")==0)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kC_Load() No extension found");
		k_EmptyString(&str);
		returnValue=false;
	}
	else
	{
		...
		if (fp==NULL)
		{
			...
			k_InitString(&str);
			k_AddFileAndLine(str);
			k_ShowMessage(IPL_ERROR,&str,"kC_Load() Could not open file");
			k_EmptyString(&str);
			returnValue=false;
		}
		else
		...
		if (returnValue==true)
		{
			...
			/* add comment to history */
			{
				TString History;
				k_InitString(&History);
				k_SprintfString(&History,"%skC_Load(%s)",ipl_HistoryIndent,FileName);
...
bool kC_Save(const char* FileName, TComplexImage* pInfo)
{
	...
	if (!k_SplitFileName(FileName, &Path,&FileNameExt,&Ext))
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kC_Save() Filename not valid");
		k_EmptyString(&str);
		return false;
	}
	else if (strcmp(Ext,"")==0)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kC_Save() No extension found");
		k_EmptyString(&str);
		returnValue=false;
	}
	else
	{
		...
		if (fp==NULL)
		{
			...
			k_InitString(&str);
			k_AddFileAndLine(str);
			k_ShowMessage(IPL_ERROR,&str,"kC_Save() Could not save file");
			k_EmptyString(&str);
			returnValue=false;
		}
		else if (pInfo->Origin==EMPTY)
		{
			...
			k_InitString(&str);
			k_AddFileAndLine(str);
			k_ShowMessage(IPL_ERROR,&str,"kC_Save() No image - cannot save");
			k_EmptyString(&str);
			returnValue=false;
		}
		else
		{
			...
			/* add comment to history */
			{
				TString History;
				k_InitString(&History);
				k_SprintfString(&History,"%skC_Save(%s)",ipl_HistoryIndent,FileName);
				k_AppendTextIPL(&(pInfo->History),History.pChars);
				k_EmptyString(&History);
			}
#endif
...
bool kC_SetPeriodicBorder(unsigned short BorderSize,TComplexImage* pInfo)
{
	if (pInfo->Origin==EMPTY)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kC_SetPeriodicBorder() Image is empty");
		k_EmptyString(&str);
		return false;
	}
	else if (((unsigned int)BorderSize>pInfo->Width) || ((unsigned int)BorderSize>pInfo->Height))
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kC_SetPeriodicBorder() Border too large");
		k_EmptyString(&str);
		return false;
	}
	else
...
bool kC_CopyImage(TComplexImage* pDest,const TComplexImage* pSource)
{
	if (pDest==pSource)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kC_CopyImage() Source and destination image"
	...
	if (pSource->Origin!=EMPTY)
	{
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%skC_CopyImage() Source image file info: %s"
					 ...
					 TComplexImage* pDest,const TComplexImage* pSource)
{
	...
	if (pSource->Origin==EMPTY)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_WARNING,&str,"kC_CopySubImage() Source image is empty - doing nothing");
		k_EmptyString(&str);
		return false;
	}
		...
		(xmax<=kC_GetMaxX(pSource)) && (ymax<=kC_GetMaxY(pSource)))
	{
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%skC_CopySubImage(xmin=%d,ymin=%d,xmax=%d,ymax=%d) Source image file info: %s"
	...
	else
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kC_CopySubImage() Illegal clipping rectangel");
		k_EmptyString(&str);
		return false;
	}
}
...
void kC_Rotate90(int Steps,TComplexImage* pSource)
{
	...
	if (pSource->Origin==EMPTY)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_WARNING,&str,"kC_Rotate90() Image is empty - doing nothing");
		k_EmptyString(&str);
		return;
	}
	if (ActSteps==0)
	...
	/* add comment to history */
	{
		TString History;
		k_InitString(&History);
		k_SprintfString(&History,"%skC_Rotate90(Steps=%d)",ipl_HistoryIndent,Steps);
	...
	else if (ActSteps==2)
	{
		for(y=0; y<(int)pSource->Height; y++)
		{
			for(x=0; x<(int)pSource->Width; x++)
			{
				kC_SetPixelMacroRe(x,MaxYIndex-y,kC_GetPixelMacroRe(x,y,(*pSource)),NewImage);
				kC_SetPixelMacroIm(x,MaxYIndex-y,kC_GetPixelMacroIm(x,y,(*pSource)),NewImage);
			}
		}
	}
	else if (ActSteps==3)
...
void kC_FlipHorisontal(TComplexImage* pSource)
{
	...
	if (pSource->Origin==EMPTY)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_WARNING,&str,"kC_FlipHorisontal() Image is empty - doing nothing");
		k_EmptyString(&str);
		return;
	}
	...
	/* add comment to history */
	{
		TString History;
		k_InitString(&History);
		k_SprintfString(&History,"%skC_FlipHorisontal()",ipl_HistoryIndent);
...
void kC_FlipVertical(TComplexImage* pSource)
{
	...
	if (pSource->Origin==EMPTY)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_WARNING,&str,"kC_FlipVertical() Image is empty - doing nothing");
		k_EmptyString(&str);
		return;
	}
	...
	/* add comment to history */
	{
		TString History;
		k_InitString(&History);
		k_SprintfString(&History,"%skC_FlipVertical()",ipl_HistoryIndent);
		k_AppendTextIPL(&(NewImage.History),History.pChars);
		k_EmptyString(&History);
		k_IplStartHistoryMacro();
	}
...
{ 
	if (pImg->Origin!=EMPTY)
	{
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%skC_ApplyBias() Bias=%g",ipl_HistoryIndent,Bias);
	...
	else
	{
        ...
        k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kC_ApplyBias(): Image is empty");
		k_EmptyString(&str);
		return false;
	}
}
...
{ 
	if (pImg->Origin!=EMPTY)
	{
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%skC_ScaleIntensity() Prefactor=%g",ipl_HistoryIndent,Prefactor);
			k_AppendTextIPL(&(pImg->History),History.pChars);
			k_EmptyString(&History);
			k_IplStartHistoryMacro();
		}
	...
	else
	{
        ...
        k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kC_ScaleIntensity(): Image is empty");
		k_EmptyString(&str);
		return false;
	}
}
...
{ 
		...
		(pImg->Width!=pAddend->Width) || (pImg->Height!=pAddend->Height))
    {
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kC_Add(): Images are empty or of different size");
		k_EmptyString(&str);
		return false;
    } 
	else
    {
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%skC_Add() Adding a TComplexImage",ipl_HistoryIndent);
			k_AppendTextIPL(&(pImg->History),History.pChars);
			k_EmptyString(&History);
			k_IplStartHistoryMacro();
		}
...
{ 
		...
		|| (pImg->Width!=pSubtrahend->Width) || (pImg->Height!=pSubtrahend->Height))
    {
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kC_Subtract(): Images are empty or of different size");
		k_EmptyString(&str);
		return false;
    } 
	else
    {
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%skC_Subtract() subtracting a TComplexImage",ipl_HistoryIndent);
			k_AppendTextIPL(&(pImg->History),History.pChars);
			k_EmptyString(&History);
			k_IplStartHistoryMacro();
		}
...
{ 
		...
		|| (pImg->Width!=pFactor->Width) || (pImg->Height!=pFactor->Height))
    {
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kC_Multiply(): Images are empty or of different size");
		k_EmptyString(&str);
		return false;
    } 
	else
    {
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%skC_Multiply() multiplying a TComplexImage",ipl_HistoryIndent);
			k_AppendTextIPL(&(pImg->History),History.pChars);
			k_EmptyString(&History);
			k_IplStartHistoryMacro();
		}
...
{ 
		...
		|| (pImg->Width!=pFactor->Width) || (pImg->Height!=pFactor->Height))
    {
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kC_CCFactorAndMultiply(): Images are empty or of different size");
		k_EmptyString(&str);
		return false;
    }
	else
    {
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%skC_CCFactorAndMultiply()",ipl_HistoryIndent);
			k_AppendTextIPL(&(pImg->History),History.pChars);
			k_EmptyString(&History);
			k_IplStartHistoryMacro();
		}
...
{ 
	if (pImg->Origin==EMPTY)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kC_ComplexConjugate(): Image is empty");
		k_EmptyString(&str);
		return false;
	}
	else
	{
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%skC_ComplexConjugate()",ipl_HistoryIndent);
			k_AppendTextIPL(&(pImg->History),History.pChars);
			k_EmptyString(&History);
			k_IplStartHistoryMacro();
		}
...
{ 
	if (pImg->Origin==EMPTY)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kC_Rerrange(): image is empty");
		k_EmptyString(&str);
		return false;
	}
	else
	{
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%skC_Rearrange()",ipl_HistoryIndent);
			k_AppendTextIPL(&(pImg->History),History.pChars);
			k_EmptyString(&History);
			k_IplStartHistoryMacro();
		}
...
bool kC_CopyDisplace(int dx,int dy,TComplexImage *pImg)
{
	if (pImg->Origin!=EMPTY)
	{
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%skC_CopyDisplace() dx=%d dy=%d",ipl_HistoryIndent,dx,dy);
	...
	else
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kC_CopyDisplace(): Image is empty");
		k_EmptyString(&str);
		return false;
	}
}
...
bool kC_CutOrZeroPad(TComplexImage *pSource, TComplexImage*pDest)
{
		...
		(ws>0) && (hs>0) && (wd>0) && (hd>0) && (pSource!=pDest))
	{
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%skC_CutOrZeroPad() Source image file info: %s",
				ipl_HistoryIndent,pSource->FileInfo.PathAndFileName);
			k_AppendTextIPL(&(pDest->History),History.pChars);
			k_EmptyString(&History);
			k_IplStartHistoryMacro();
	...
	else
	{
		...
		if (pSource==pDest)
		{
			k_ShowMessage(IPL_ERROR,&str,"kC_CutOrZeroPad(): Cannot use same image as dest/source"); 
		}
		else if ((ws<=0) || (hs<=0) || (wd<=0) || (hd<=0))
		{
			k_ShowMessage(IPL_ERROR,&str,"kC_CutOrZeroPad(): One or both images have no pixels"); 
		}
		else
		{
			k_ShowMessage(IPL_ERROR,&str,"kC_CutOrZeroPad(): One or both images are empty");
		}
		k_EmptyString(&str);
		return false;
	}
...
bool kC_CopyFromByte(TComplexImage *pDest, TImage *pRe, TImage *pIm)
{
	...
	if ((pRe->Origin==EMPTY) || (pIm->Origin==EMPTY) || (pRe->Width!=pIm->Width) || (pRe->Height!=pIm->Height))
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kC_CopyFromByte(): Source images are not same size or empty");
		k_EmptyString(&str);
		k_EmptyString(&str);
		return false;
	}
	else
	{
...
#ifdef IPL_ADD_HISTORY
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%skC_CopyFromByte() History from pRe",ipl_HistoryIndent);
			k_AppendTextIPL(&(pDest->History),History.pChars);
			k_EmptyString(&History);
			k_IplStartHistoryMacro();
		}
		...
		k_IplStopHistoryMacro();
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%skC_CopyFromByte() History from pIm",ipl_HistoryIndent);
			k_AppendTextIPL(&(pDest->History),History.pChars);
			k_EmptyString(&History);
			k_IplStartHistoryMacro();
		}
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%skC_CopyFromByte()",ipl_HistoryIndent);
			k_AppendTextIPL(&(pDest->History),History.pChars);
			k_EmptyString(&History);
			k_IplStartHistoryMacro();
		}
...
bool kC_CopyFromInt(TComplexImage *pDest, TIntImage *pRe, TIntImage *pIm)
{
	if ((pRe->Origin==EMPTY) || (pIm->Origin==EMPTY) || (pRe->Width!=pIm->Width) || (pRe->Height!=pIm->Height))
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kC_CopyFromInt():  Source images are not same size or empty");
		k_EmptyString(&str);
		k_EmptyString(&str);
		return false;
	}
	else
	{
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%skC_CopyFromInt()",ipl_HistoryIndent);
			k_AppendTextIPL(&(pDest->History),History.pChars);
			k_EmptyString(&History);
			k_IplStartHistoryMacro();
		}
...
bool kC_CopyFromFloat(TComplexImage *pDest, TFloatImage *pRe, TFloatImage *pIm)
{
	if ((pRe->Origin==EMPTY) || (pIm->Origin==EMPTY) || (pRe->Width!=pIm->Width) || (pRe->Height!=pIm->Height))
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kC_CopyFromFloat():  Source images are not same size or empty");
		k_EmptyString(&str);
		k_EmptyString(&str);
		return false;
	}
	else
	{
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%skC_CopyFromFloat()",ipl_HistoryIndent);
			k_AppendTextIPL(&(pDest->History),History.pChars);
			k_EmptyString(&History);
			k_IplStartHistoryMacro();
		}
...
void kC_PrintImageInfo(const TComplexImage* pInfo)
{
	...
	printf(" Width=%d Height=%d ByteWidth=%d SizeImage=%d\n",
...
bool kC_DrawLine(T2DInt Start, T2DInt End, FLOAT32 Value, TComplexImage *pDest)
{
	...
	if (pDest==NULL)
	{
        ...
        k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kC_DrawLine(): Image is empty");
		k_EmptyString(&str);
		return false;
	}
	k_InitGroup(&grp);
...
FLOAT32 kC_GetPixelRe(int x, int y,const TComplexImage* pInfo)
{
	...
	else
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kC_GetPixel(%d,%d) Outside image and border",x,y);
		k_EmptyString(&str);
		return 0;
	}
}
...
bool kC_SetPixel(int x, int y, FLOAT32 Re, FLOAT32 Im,TComplexImage* pInfo)
{
	...
	else
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kC_SetPixel(%d,%d) Outside image and border",x,y);
		k_EmptyString(&str);
		return false;
	}
}
...
bool kC_SetPixelRe(int x, int y, FLOAT32 Re,TComplexImage *pInfo)
{
	...
	else
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kC_SetPixelRe(%d,%d) Outside image and border",x,y);
		k_EmptyString(&str);
		return false;
	}
}
...
bool kC_SetPixelIm(int x, int y, FLOAT32 Im,TComplexImage *pInfo)
{
	...
	else
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kC_SetPixelIm(%d,%d) Outside image and border",x,y);
		k_EmptyString(&str);
		return false;
	}
}
...
bool kC_LoadComplex(FILE* fp,TComplexImage* pInfo)
{
	...
	while (fgetc(fp)=='#')
	{
		if ((fgetc(fp)!='#') && (TokenRead==false))
		{
			...
			if (CharsInLine==-1)
			{
				...
				k_InitString(&str);
				k_AddFileAndLine(str);
				k_ShowMessage(IPL_ERROR,&str,"kC_LoadFloat() wrong header - "
		...
		else
		{
			...
			if (Token==NULL)
			{
				...
				k_InitString(&str);
				k_AddFileAndLine(str);
				k_ShowMessage(IPL_ERROR,&str,"kC_LoadComplex() wrong file format, end of file reached");
				k_EmptyString(&str);
				return false;
			}
			else if (strcmp(Token,"WidthHeight")==0)
			{
				...
				if (fscanf(fp,"%d %d",&Width,&Height)!=2)
				{
					...
					k_InitString(&str);
					k_AddFileAndLine(str);
					k_ShowMessage(IPL_ERROR,&str,"kC_LoadComplex() wrong header - "
				...
				if (WidthHeightRead==true)
				{
					...
					if ((Width!=Cols) || (Height!=Rows))
					{
						...
						k_InitString(&str);
						k_AddFileAndLine(str);
						k_ShowMessage(IPL_ERROR,&str,"kC_LoadComplex() Width=Cols (%d=%d) and height=Rows (%d,%d) "
							"does not match",Width,Cols,Height,Cols,Rows);
						k_EmptyString(&str);
						return false;
					}
				}
	...
	if (WidthHeightRead==false)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kC_LoadComplex() wrong header - width and height not found");
		k_EmptyString(&str);
		return false;
	}
	/* allocate image */
		...
		for(x=0;x<Width;x++)
		{
			if (k_SkipWhiteSpace(fp)!=-1)
			{
				...
				else
				{
					...
					k_InitString(&str);
					k_AddFileAndLine(str);
					k_ShowMessage(IPL_ERROR,&str,"kC_LoadComplex() Failed reading real values");
					k_EmptyString(&str);
					kC_EmptyImage(pInfo);
					return false;
				}
		...
		for(x=0;x<Width;x++)
		{
			if (k_SkipWhiteSpace(fp)!=-1)
			{
				...
				else
				{
					...
					k_InitString(&str);
					k_AddFileAndLine(str);
					k_ShowMessage(IPL_ERROR,&str,"kC_LoadComplex() Failed reading imaginar values");
					k_EmptyString(&str);
					kC_EmptyImage(pInfo);
					return false;
				}
...
bool kC_SaveComplex(FILE* fp,TComplexImage* pInfo)
{
	...
	if (pInfo->Origin==EMPTY)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_WARNING,&str,"kC_SaveComplex() No image to save");
		k_EmptyString(&str);
		fclose(fp);
		return false;
	}
