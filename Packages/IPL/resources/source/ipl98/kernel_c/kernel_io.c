...
bool k_CopyFileInfo(TFileInfo* pDest,const TFileInfo* pSource)
{
	if (pDest==pSource)
	{
		...
		k_ShowMessage(IPL_ERROR,&str,"k_CopyFileInfo() Source and destination file info"
	...
	if (k_SkipWhiteSpace(fp)!=-1) /* set fp to the first non whitespace character */
	{
		...
		*ppToken=(char*)malloc(count+1);
		if (fseek(fp,-(count+1),SEEK_CUR)!=0)
		{
			...
			k_InitString(&str);
			k_AddFileAndLine(str);
			k_ShowMessage(IPL_ERROR,&str,"k_ReadToken() Internal error (fseek) when reading a token from file");
			k_EmptyString(&str);
			return false;
		}
		if (fread(*ppToken,sizeof(char),count,fp)!=(unsigned int)count)
		{
			...
			k_InitString(&str);
			k_AddFileAndLine(str);
			k_ShowMessage(IPL_ERROR,&str,"k_ReadToken() Internal error (fread) when reading a token from file");
			k_EmptyString(&str);
			return false;
		}
...
bool k_SetFileAndPath(const char* FileAndPathName,TFileInfo* pInfo)
{
	...
	if (k_SplitFileName(FileAndPathName,&pInfo->PathName,&pInfo->FileName,&Ext)==false)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_SetFileAndPath() Failed setting variables in TFileInfo structure");
	    k_EmptyString(&str);
		if (Ext!=NULL)
		{
			free(Ext);
...
bool k_SplitFileName(const char* FilePathName, char** Path, char** FileNameExt, char** Ext)
{
	...
	if ((FilePathName==NULL) || (strcmp(FilePathName,"")==0))
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_WARNING,&str,"k_SplitFileName() The given file name is empty - doing nothing");
       	    k_EmptyString(&str);
		return false;
	}
	...
	if (UFilePathName[strlen(UFilePathName)-1]=='/')
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_SplitFileName() The given file name ends with a '/' character");
	    k_EmptyString(&str);
		returnValue=false;
	}
	else
	{
		...
		/*printf("UFilePathName=%s UFileNameCopy=%s\n",UFilePathName,UFileNameCopy);*/
