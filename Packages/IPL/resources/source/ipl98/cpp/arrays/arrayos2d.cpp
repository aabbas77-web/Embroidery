...
bool CArrayOS2D<T>::Load(const char* pPathAndFileName)
{
	...
	if (s.is_open()==true)
	{
		...
		SkipSpaceAndComments(s);
		s >> m_SubIndexing;
		Alloc(m_Width,m_Height,m_SubIndexing);
		SetOrigo(Origo);
...
//	double Step=GetSubIndexing();
	int ixmin=Source.GetNearestSubIndex(xmin)+Source.GetOrigo().GetX()*Step;
	int ixmax=Source.GetNearestSubIndex(xmax)+Source.GetOrigo().GetX()*Step;
	int iymin=Source.GetNearestSubIndex(ymin)+Source.GetOrigo().GetY()*Step;
	int iymax=Source.GetNearestSubIndex(ymax)+Source.GetOrigo().GetY()*Step;
	if ((xmin<xmax) && (ymin<ymax) && 
		(ixmin>=0) && (ixmax<=Source.GetWidthCells()-1) &&
		(iymin>=0) && (iymax<=Source.GetHeightCells()-1))
	{
