...
/** Derived from CArray2D this class offers subindexing and origo, last updated 1/25/2002.
	...
	@version 0.35,  by edr@mip.sdu.dk
...
public:
		...
		/** Returns an iterator for column i. Use another [] to index
			...
			@see Get
			@see Set */
        inline Iterator operator[](double i) { return Iterator(m_pData,slice((i+m_Origo.GetX())*m_SubIndexing*m_Height,m_Height,1),m_SubIndexing,&m_Origo); }   // C-style subscript
		/** Returns a const iterator for column i. Use another [] to index
			...
			@see Get
			@see GetFast */
        inline ConstIterator operator[](double i) const { return ConstIterator(m_pData,slice((i+m_Origo.GetX())*m_SubIndexing*m_Height,m_Height,1),m_SubIndexing,&m_Origo); }
		...
		/** Copies a CArrayOS2D to this array.
			...
			@param LowerLeft position in the lower left corner to stop at (including value).
			@return false if outside array */
		...
		friend ostream& operator<<(ostream& s,const CArrayOS2D<T>& TS);
		/** Writes all data to stream. Format is:
			...
			@version 0.3 */
		friend ostream& operator<<(ostream& s, const CArrayOS2D<T>& Array)
		{
			s << "Dimensions: " << Array.GetWidth() << " " << Array.GetHeight() << endl;
			s << "Origo: " << Array.GetOrigo() << endl;
			s << "SubIndexing: " << Array.GetSubIndexing() << endl;
...
protected:
		/** @internal Sets the attribute m_SubIndexing, and updates the corresponding
			attribute m_SubStep.
...
inline CArrayOS2D<T>& CArrayOS2D<T>::operator=(const CArrayOS2D<T>& M)
{
	...
	if ((M.GetWidth()!=0) && (M.GetHeight()!=0))
	{
		...
		m_pData = new valarray<T>(m_SubIndexing*m_Width*m_SubIndexing*m_Height);
		*m_pData=M.Array();
	}
	return *this;
}
...
{
	return m_Width/m_SubIndexing;
}

template <class T>
inline UINT32 CArrayOS2D<T>::GetHeight() const
{
	return m_Height/m_SubIndexing;
}
...
inline void CArrayOS2D<T>::SetOrigo(const CPoint2D<double>& Origo)
{
	...
	m_OrigoCells.SetX(x*m_SubIndexing);
	m_OrigoCells.SetY(y*m_SubIndexing);
	m_Origo.Set(x,y);
}
...
{
	return ((double)m_Width-m_OrigoCells.GetX())/(double)m_SubIndexing;
}

template<class T> 
inline double CArrayOS2D<T>::GetMaxY() const
{
	return ((double)m_Height-m_OrigoCells.GetY())/(double)m_SubIndexing;
}
...
inline bool CArrayOS2D<T>::InRange(double x, double y) const
{
	...
	return ((ix>=0) && (ix<=m_Width-1) && (iy>=0) && (iy<=m_Height-1));
//	x = x*m_SubIndexing + m_OrigoCells.GetX();
//	y = y*m_SubIndexing + m_OrigoCells.GetY();
...
inline bool CArrayOS2D<T>::InRange(const CPoint2D<double>& P) const
{
	...
	return ((ix>=0) && (ix<=m_Width-1) && (iy>=0) && (iy<=m_Height-1));
//	double x = P.GetX()*m_SubIndexing + m_OrigoCells.GetX();
//	double y = P.GetY()*m_SubIndexing + m_OrigoCells.GetY();
//	return ((x>=0) && (x<m_Width) && (y>=0) && (y<m_Height));
}

template <class T>
inline CArrayOS2D<T>::Iterator CArrayOS2D<T>::Row(double i)
{
	return Iterator(m_pData,slice((i+m_Origo.GetX())*m_SubIndexing,m_Width,m_Height),m_SubIndexing,&m_Origo);
}

template <class T>
inline CArrayOS2D<T>::ConstIterator CArrayOS2D<T>::ConstRow(double i) const
{
	return ConstIterator(m_pData,slice((i+m_Origo.GetX())*m_SubIndexing,m_Width,m_Height),m_SubIndexing,&m_Origo);
}

template <class T>
inline CArrayOS2D<T>::Iterator CArrayOS2D<T>::Column(double i)
{
	return Iterator(m_pData,slice((i+m_Origo.GetX())*m_SubIndexing*m_Height,m_Height,1),m_SubIndexing,&m_Origo);
}

template <class T>
inline CArrayOS2D<T>::ConstIterator CArrayOS2D<T>::ConstColumn(double i) const
{
	return ConstIterator(m_pData,slice((i+m_Origo.GetX())*m_SubIndexing*m_Height,m_Height,1),m_SubIndexing,&m_Origo);
}
