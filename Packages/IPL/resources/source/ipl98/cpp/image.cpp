...
bool CImage::Load(const char* pPathAndFileName)
{
	...
	k_SprintfString(&History,"%sCStdImage::Load(%s)",ipl_HistoryIndent,pPathAndFileName);
...
bool CImage::CopyConvert(UINT16 Bits,CStdImage& SourceImage)
{
	...
	TString History;
	k_InitString(&History);
	k_SprintfString(&History,"%sCStdImage::CopyConvert() From %d b/p to %d b/p, source image file info: %s",
...
bool CImage::CopyConvertThreshold(UINT8 Threshold,CStdImage& SourceImage)
{
	...
	TString History;
	k_InitString(&History);
	k_SprintfString(&History,"%sCStdImage::CopyConvertThreshold() From 8 b/p to 1 b/p (always), source image file info: %s",
		ipl_HistoryIndent,SourceImage.GetConstTImagePtr()->FileInfo.PathAndFileName);
	SourceImage.m_History.AppendIPL(History.pChars);
	k_EmptyString(&History);
	k_IplStartHistoryMacro();
#endif
