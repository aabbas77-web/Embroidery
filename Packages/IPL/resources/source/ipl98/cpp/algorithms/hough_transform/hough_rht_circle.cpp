...
{
	m_bImage = CBinaryImage(Source, Color, Threshold, OriginType);
	return this->InitializeCommon();
}


bool CHoughRHTCircle::Initialize(const CBinaryImage& Source, CHoughBase::ORIGINTYPE OriginType)
{
	m_bImage = Source;
	return this->InitializeCommon();
}
...
vector<CPoint3D<int> > CHoughRHTCircle::PerformTransform(int Threshold)
{
	...
	{
		this->GetPixelTriple();
		CPoint3D<int> parameter(this->RH_Circle_Transform()); // changed to CPoint3D<int> from float by edr
...
CPoint3D<float> CHoughRHTCircle::RH_Circle_Transform()
{
	...
	if (d != 0)
	{
		...
		c1 = c1/2/d;
		c2 = c2/2/d;
		c3 = abs(sqrt(1.0*c31)*sqrt(1.0*c32)*sqrt(1.0*c33)/2/d);
	}
