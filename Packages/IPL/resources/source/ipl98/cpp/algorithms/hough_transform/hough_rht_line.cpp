...
bool CHoughRHTLine::Initialize(CBinaryImage& BinarySource, int Thickness, 
...
bool CHoughRHTLine::Initialize(const CStdImage& Source, COLORTYPE Color, UINT8 Threshold, 
   ...
   m_binaryImage = CBinaryImage(Source, Color, Threshold, OriginType);
   return this->InitializeCommon(Thickness, SpaceType, ParamType);
}
...
vector<CParameter2D<float> > CHoughRHTLine::PerformTransform(int HSThreshold, int MaxNumShapes) {
   ...
   m_os << "Detection threshold:      " << HSThreshold << endl;
	...
	CBinaryImage rc_bImg(m_binaryImage.GetWidth(), m_binaryImage.GetHeight(), m_binaryImage.GetOrigin());
...
void CHoughRHTLine::WriteFiles(CBinaryImage& BImage, const CParameter2D<float>& Coordinate, int NumOfShapes) {
   string filename;
   char no[2];
...
float CHoughRHTLine::Distance() {
   float dx12 = m_p1.GetX() - m_p2.GetX();
   float dy12 = m_p1.GetY() - m_p2.GetY();
