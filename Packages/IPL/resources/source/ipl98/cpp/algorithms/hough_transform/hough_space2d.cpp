	...
	int Thickness, CHoughBase::PARAMETERTYPE ParamType)
{
	...
	D = (float) sqrt(pow(ImageWidth,2.0)+pow(ImageHeight,2.0));
	
	switch (ParamType)
	{
	case CHoughBase::NORMAL_PARAM:
		deltaRho = 2*error + Thickness + 1;
		HRho = ceil(D/2.0f/deltaRho);
		nTheta = ceil(PI/2.0f*D/deltaRho);
		nRho = 2*HRho+1;
...
vector<CParameter2D<float> > CHoughSpace2D::FindPeaks_Diffusion()
{
	...
	while (p > 10)
	{
		...
		if (max > m_threshold)
		{
			...
			if (0<xi && xi<GetWidth()-1 && 0<yi && yi<GetHeight()-1)
			{
...
bool CHoughSpace2D::CellResetTest(int x, int y, unsigned int from_value, DIRECTION direction)
{
	...
	{        // range test
		...
		if (m_threshold<this_value && from_value-this_value>=-noise_tol)
		{														// gradient test
...
vector<CParameter2D<float> > CHoughSpace2D::FindPeaks_Butterfly()
{
	...
	for (int y=0; y<newspace.GetHeight(); y++)
	{
		for (int x=0; x<newspace.GetWidth(); x++)
		{
			if (newspace[x][y] == 1)
			{
				m_peakCoord.Set(x,y);
