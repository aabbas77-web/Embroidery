...
bool CHoughProbRHTLine::Initialize(float Ratio, CBinaryImage& Source, int Thickness, 
...
bool CHoughProbRHTLine::Initialize(float Ratio, const CStdImage& Source, COLORTYPE Color, UINT8 Threshold, 
                                   ...
                                   CHoughBase::PARAMETERSPACETYPE SpaceType, CHoughBase::PARAMETERTYPE ParamType)
{
	CBinaryImage InImage(Source, Color, Threshold, OriginType);
...
void CHoughProbRHTLine::PerformSelection(float Ratio, CBinaryImage& Source)
{
	if (0.0f<Ratio && Ratio<=1.0f)
	{
		int k = (1.0-Ratio)*Source.GetSize();
