...
/** CGraphics contains two kind of functionalities: 1) A collection of algorithms 
	...
	Two fonts are available at the moment: CourierBold13x18 and Terminal12x16. The fonts
	are only allocated when used, i.e. constructing a CGraphics object and not using the
	fonts does not spend time on allocating the fonts! See the font method section for
	available methods. Note: Font methods are only available in C++.
...
class CGraphics{
	...
	public:  // methods
		...
		/** Generates a discrete set of points on a line described by the given parameters,
			...
			@param PixelGroup The group to store the positions in.
