...
/** CPositionGroup2D, contains a group of positions, last updated 1/26/2002.
...
class CPositionGroup2D {
	...
	public: // methods
		...
		/** Removes a position stored in the group at given index. Done by moving the
			last position in the m_Positions array to the index which is to be removed.
			The boundary is also updated if the removed position causes changes in the
			boundary.
		...
		/** Load positions contained in file given by pPathAndFileName. Format must
			...
			streaming operators. See general description for this class for more info.
		...
		/** writes all the group information to stream */
		friend ostream& operator<<(ostream& s,const CPositionGroup2D& Group);
...
{
	return m_Right.GetX()-m_Left.GetX()+1;
}

template<class T> 
inline unsigned int CPositionGroup2D<T>::GetHeight() const
{
	return m_Bottom.GetY()-m_Top.GetY()+1;
}
