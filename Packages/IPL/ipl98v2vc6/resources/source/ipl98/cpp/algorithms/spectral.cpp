...
bool CSpectral::FFTForward(CComplexImage &Img)
{
	...
	/* add comment to history */
	{
		...
		k_SprintfString(&History,"%sCSpectral::FFTForward()",ipl_HistoryIndent);
...
bool CSpectral::FFTInverse(CComplexImage &Img)
{
	...
	/* add comment to history */
	{
		TString History;
		k_InitString(&History);
		k_SprintfString(&History,"%CSpectral::FFTInverse()",ipl_HistoryIndent);
		Img.m_History.AppendIPL(History.pChars);
		k_EmptyString(&History);
		k_IplStartHistoryMacro();
	}
...
bool CSpectral::DeriveCorrelation(const CStdImage &Lg,const CStdImage &Sm, CFloatImage &Dest)
{
	...
	for (x=0;x<wL;x++)
		for (y=0;y<hL;y++)
			CLg.SetPixelFastRe(x,y,Lg.GetPixelFast(x,y));
	...
	for (x=0;x<wS;x++)
		for (y=0;y<hS;y++)		
			CSm.SetPixelFastRe(x,y,Sm.GetPixelFast(x,y));
