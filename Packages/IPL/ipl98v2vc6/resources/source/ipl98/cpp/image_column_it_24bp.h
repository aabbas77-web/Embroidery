...
class CImageColumnIterator24bp
{
	...
	class CIntermediateType
	{
		...
		/** Called when CImageColumnIterator24bp::operator[] is used. */
		inline const UINT32 Get(UINT32 i,UINT16 ByteWidth){return *(UINT32*)(*m_ppCurr-i*ByteWidth) & 0xffffff;}
	};
...
public:
	...
	/** overloading [] operator. */
	inline const UINT32 operator[](UINT32 i) {return m_IntermediateType.Get(i,m_pTImg->ByteWidth);}
	/** overloading == operator.
...
inline CImageColumnIterator24bp::CImageColumnIterator24bp(UINT32 ColumnIndex, UINT32 StartIndex, UINT32 MaxIndex,
															const TImage* pTImg):
		m_pTImg(pTImg), m_pCurr(pTImg->ppAllScanLines[StartIndex]+ColumnIndex*3),
