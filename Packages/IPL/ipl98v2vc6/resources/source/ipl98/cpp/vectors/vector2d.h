...
class CVector2D : public CPoint2D<T>
{
	...
	public: // methods
		...
		template<class POLY> inline CVector2D(const CVector2D<POLY>& V2D){m_x=V2D.GetX();m_y=V2D.GetY();}
		/// copy constructor of CPoint2D with template conversion
		template<class POLY> inline CVector2D(const CPoint2D<POLY>& P2D){m_x=P2D.GetX();m_y=P2D.GetY();}

		/// cast to POLY in mixed expressions
		template<class POLY> operator CVector2D<POLY>(){return CVector2D<POLY>(m_x,m_y);}

		/**@name overloading assignment operators */
		//@{
		/// overloading = operator with template conversion
		template<class POLY> inline CVector2D<T>& operator=(const CVector2D<POLY>& V2D){m_x=V2D.GetX();m_y=V2D.GetY();return *this;};
		/// overloading += operator with template conversion
		template<class POLY> inline CVector2D<T>& operator+=(const CVector2D<POLY>& V2D){m_x+=V2D.GetX();m_y+=V2D.GetY();return *this;};
		/// overloading -= operator with template conversion
		template<class POLY> inline CVector2D<T>& operator-=(const CVector2D<POLY>& V2D){m_x-=V2D.GetX();m_y-=V2D.GetY();return *this;};
		...
		template<class POLY> inline CVector2D<T>& operator*=(const POLY& Factor){m_x*=Factor;m_y*=Factor;return *this;}
		/** overloading /= operator with template conversion
		...
		//@{
		/// overloading == operator with template conversion
		template<class POLY> inline bool operator==(const CVector2D<POLY>& V2D) const{return ((m_x==V2D.GetX()) && (m_y==V2D.GetY()));}
		/// overloading != operator with template conversion
		template<class POLY> inline bool operator!=(const CVector2D<T>& V2D) const{return ((m_x!=V2D.GetX()) || (m_y!=V2D.GetY()));}
		//@}
		/**@name overloading arithmetic operators */
		//@{
		/// overloading + operator with template conversion
		template<class POLY> inline CVector2D<T> operator+(const CVector2D<POLY>& V2D){return CVector2D<T>(m_x+V2D.GetX(),m_y+V2D.GetY());}
		/// overloading - operator with template conversion
		template<class POLY> inline CVector2D<T> operator-(const CVector2D<POLY>& V2D){return CVector2D<T>(m_x-V2D.GetX(),m_y-V2D.GetY());}
		/** overloading * operator used for dot product. If you want the dot product
			...
			@see GetDot */
		inline T operator*(const CVector2D<T>& Vector) const {return (m_x*Vector.GetX()+m_y*Vector.GetY());}
		/** overloading * operator with template conversion
			 @param Factor Multiply each dimension with this value */
		template<class POLY> inline CVector2D<T> operator*(const POLY& Factor) const {return CVector2D<T>(m_x*Factor,m_y*Factor);}
		/** overloading / operator with template conversion
			 @param Factor Divide each dimension with this value */
		template<class POLY> inline CVector2D<T> operator/(const POLY& Factor) const {return CVector2D<T>(m_x/Factor,m_y/Factor);}
		//@}
		
		/** a friend method overloading the * operator, makes it possible to write f*V2D,
			where f is a factor of type T and V2D is of type CVector2D<V>. For some reason
			this cannot be converted as other operators, i.e. f and V2D must contain same types. */
		friend CVector2D<T> operator*(const T& Factor,const CVector2D<T>& V2D);
		
		
		/** Returns the dot product between this vector and the parameter Vector.
			@param Vector Calculate dot product with this parameter.
			@return Dot product between the two vectors. */
		template<class POLY> inline T GetDot(const POLY& Vector) const {return (m_x*Vector.GetX()+m_y*Vector.GetY());}
		...
		/** Returns the angle in radians between this vector and the supplied vector.
			...
			@return Angle in radians, range ]-PI,PI] */
		template<class POLY> inline double GetAngle(const CVector2D<POLY>& Vec) const {T p=GetDot(Vec)/(GetModulus()*Vec.GetModulus());if ((m_x*Vec.GetY()-m_y*Vec.GetX())>=0) {return atan2(sqrt(1-p*p),p);}else{return -atan2(sqrt(1-p*p),p);}}
		...
		template<class POLY> inline double GetDeterminant(const CVector2D<POLY>& Vec) const {return m_x*Vec.GetY() - m_y*Vec.GetX();}
		...
		inline bool Normalize(){T L=GetModulus();if (L!=0){m_x/=L;m_y/=L;return true;}else{return false;}}
};
