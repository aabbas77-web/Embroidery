...
bool CCorresponding3D2DPoints::GetIndex(unsigned int& Index, int Id)
{
...
bool CCorresponding3D2DPoints::Save(const char* PathAndFileName)
{
...
{
	s << "**************** CCorresponding3D2DPoints info ********************\n" <<
		" Number of point sets inserted: " << PointSets.GetTotalPointSets() <<
	...
	for(count=0; count<PointSets.GetTotalPointSets(); count++)
	{
		...
		{
			s << " 3D pnt = " << PointSets.GetPoint3D(count);
		}
		else
		{
			s << " 3D pnt = not in use";
		...
		{
			s << " 2D pnt = " << PointSets.GetPoint2D(count);
		}
		else
		{
			s << " 2D pnt = not in use";
