...
/** A 2D array to be used as a container, last updated 1/25/2002. The main
	...
	it seems to work with bot Borland 4.0 and G++ 3.0.3.
...
public:
		...
		/** Copies a CArray2D to this array.
			...
			@param LowerRight position in the lower left corner to stop at (including value).
			@return false if outside array */
		...
		/** Writes all data to stream. Format is:
			...
			Where w is width of array and h is height.
		...
		/** Reads input from given stream into this class. Stream must be in correct format
			...
			a bit (but not in all cases) to bypass the bugs.
			@version 0.3 */
		friend istream& operator>>(istream& is, ipl::CArray2D<T>& Array)
		{
			UINT32 w,h;
...
template <class T>
inline CArray2D<T>::Iterator CArray2D<T>::Row(UINT32 i)
{
...
template <class T>
inline CArray2D<T>::ConstIterator CArray2D<T>::ConstRow(UINT32 i) const
{
	return ConstIterator(m_pData,slice(i,m_Width,m_Height));
}

template <class T>
inline CArray2D<T>::Iterator CArray2D<T>::Column(UINT32 i)
{
...
template <class T>
inline const CArray2D<T>::ConstIterator CArray2D<T>::ConstColumn(UINT32 i) const
{
