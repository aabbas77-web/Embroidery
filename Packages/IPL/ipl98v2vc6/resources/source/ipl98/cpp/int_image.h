...
/** CIntImage is an abstract base class derived from CGenericImage consisting
	of pixels with type INT16, last updated 7/21/2002. Saving and loading
	...
	@see CGenericImage
	@version 1.52,  by edr@mip.sdu.dk
	...
	public: // methods
		...
		*/
		...
		/** Calculates product of two images. The border (if present) is not
			affected. Adds to the history.
			@param pFactor a pointer to the image of class CIntImage to be multiplied
			@return False if one or both of the images involved are empty */
		...
		bool CutOrZeroPad(CIntImage &Dest);
