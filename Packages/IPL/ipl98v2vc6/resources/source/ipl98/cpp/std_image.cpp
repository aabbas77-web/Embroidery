...
CStdImage& CStdImage::operator=(const CStdImage& SourceImage)
{
	...
	if (this != &SourceImage)
	{
		...
		if (SourceImage.GetOrigin()!=EMPTY)
		{
			...
			k_SprintfString(&History,"%sCStdImage::operator=",ipl_HistoryIndent);
...
bool CStdImage::Flush(UINT32 Color)
{
	...
	TString History;
	k_InitString(&History);
	k_SprintfString(&History,"%sCStdImage::Flush(Color=%d)",ipl_HistoryIndent,Color);
	m_History.AppendIPL(History.pChars);
	k_EmptyString(&History);
	k_IplStartHistoryMacro();
#endif
...
bool CStdImage::Invert()
{
	...
	TString History;
	k_InitString(&History);
	k_SprintfString(&History,"%sCStdImage::Invert()",ipl_HistoryIndent);
	m_History.AppendIPL(History.pChars);
	k_EmptyString(&History);
	k_IplStartHistoryMacro();
#endif
				  ...
				  int xmax, int ymax)
{
	...
	TString History;
	k_InitString(&History);
	k_SprintfString(&History,"%sCStdImage::CopySubImage(xmin=%d,ymin=%d,xmax=%d,ymax=%d) Source image file info: %s",
		ipl_HistoryIndent,xmin,ymin,xmax,ymax,SourceImage.GetPathAndFileName());
		  ...
		  const CPoint2D<int>& LowerRight)
{
	...
	TString History;
	k_InitString(&History);
	k_SprintfString(&History,"%sCStdImage::CopySubImage(UpperLeft=(%d,%d),LowerLeft=(%d,%d)) Source image file info: %s",
...
void CStdImage::Rotate90(int Steps)
{
	...
	TString History;
	k_InitString(&History);
	k_SprintfString(&History,"%sCStdImage::Rotate90(Steps=%d)",ipl_HistoryIndent,Steps);
	m_History.AppendIPL(History.pChars);
	k_EmptyString(&History);
	k_IplStartHistoryMacro();
#endif
...
void CStdImage::FlipHorisontal()
{
	...
	TString History;
	k_InitString(&History);
	k_SprintfString(&History,"%sCStdImage::FlipHorisontal()",ipl_HistoryIndent);
	m_History.AppendIPL(History.pChars);
	k_EmptyString(&History);
	k_IplStartHistoryMacro();
#endif
...
void CStdImage::FlipVertical()
{
	...
	TString History;
	k_InitString(&History);
	k_SprintfString(&History,"%sCStdImage::FlipVertical()",ipl_HistoryIndent);
	m_History.AppendIPL(History.pChars);
	k_EmptyString(&History);
	k_IplStartHistoryMacro();
#endif
...
bool CStdImage::ApplyBias(int Bias)
{
	...
	TString History;
	k_InitString(&History);
	k_SprintfString(&History,"%sCStdImage::ApplyBias() Bias=%d",ipl_HistoryIndent,Bias);
	m_History.AppendIPL(History.pChars);
	k_EmptyString(&History);
	k_IplStartHistoryMacro();
#endif
...
bool CStdImage::ScaleIntensity(float Prefactor)
{
	...
	TString History;
	k_InitString(&History);
	k_SprintfString(&History,"%sCStdImage::ScaleIntensity() Prefactor=%g",ipl_HistoryIndent,Prefactor);
	m_History.AppendIPL(History.pChars);
	k_EmptyString(&History);
	k_IplStartHistoryMacro();
#endif
...
bool CStdImage::Add(CStdImage &Addend)
{
	...
	TString History;
	k_InitString(&History);
	k_SprintfString(&History,"%sCStdImage::Add() adding a CStdImage",ipl_HistoryIndent);
	m_History.AppendIPL(History.pChars);
	k_EmptyString(&History);
	k_IplStartHistoryMacro();
#endif
...
bool CStdImage::Subtract(CStdImage &Subtrahend)
{
	...
	TString History;
	k_InitString(&History);
	k_SprintfString(&History,"%sCStdImage::Subtract() subtracting a CStdImage",ipl_HistoryIndent);
	m_History.AppendIPL(History.pChars);
	k_EmptyString(&History);
	k_IplStartHistoryMacro();
#endif
...
bool CStdImage::Multiply(CStdImage &Factor)
{
	...
	TString History;
	k_InitString(&History);
	k_SprintfString(&History,"%sCStdImage::Multiply() multiplying a CStdImage",ipl_HistoryIndent);
	m_History.AppendIPL(History.pChars);
	k_EmptyString(&History);
	k_IplStartHistoryMacro();
#endif
...
bool CStdImage::CopyDisplace(int dx, int dy)
{
	...
	TString History;
	k_InitString(&History);
	k_SprintfString(&History,"%sCStdImage::CopyDisplace() dx=%d dy=%d",ipl_HistoryIndent,dx,dy);
	m_History.AppendIPL(History.pChars);
	k_EmptyString(&History);
	k_IplStartHistoryMacro();
#endif
...
bool CStdImage::CutOrZeroPad(CStdImage &Source)
{
	...
	TString History;
	k_InitString(&History);
	k_SprintfString(&History,"%sCStdImage::CutOrZeroPad() Source image file info: %s",
...
bool CStdImage::Copy(CIntImage &Source)
{
	...
	TString History;
	k_InitString(&History);
	k_SprintfString(&History,"%sCStdImage::Copy() source: CIntImage",ipl_HistoryIndent);
	m_History.AppendIPL(History.pChars);
	k_EmptyString(&History);
	k_IplStartHistoryMacro();
#endif
...
bool CStdImage::Copy(CFloatImage &Source)
{
	...
	TString History;
	k_InitString(&History);
	k_SprintfString(&History,"%sCStdImage::Copy() source: CFloatImage",ipl_HistoryIndent);
	m_History.AppendIPL(History.pChars);
	k_EmptyString(&History);
	k_IplStartHistoryMacro();
#endif
