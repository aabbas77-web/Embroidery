...
/** Iterator class to access data from the class CPositionGroup2D, last updated 1/25/2002.
...
public:
	...
	/// Constructor
	CPositionGroup2DConstIterator(std::vector<T>::const_iterator It) :m_It(It){ }
	/// overloading = operator
	inline CPositionGroup2DConstIterator<T>& operator=(const CPositionGroup2DConstIterator<T>& It);
	/// overloading = operator with a CPixelGroupIterator as right operand
	inline CPositionGroup2DConstIterator<T>& operator=(const CPositionGroup2DIterator<T>& It);
	...
	friend bool operator<(const CPositionGroup2DConstIterator<T>& p, const CPositionGroup2DConstIterator<T>& q){return p.m_It<q.m_It;}
	/** overloading < operator with CPixelGroupIterator as right operand.
	@return true, if iterator p points to element earlier than q. */
	friend bool operator<(const CPositionGroup2DConstIterator<T>& p, const CPositionGroup2DIterator<T>& q){return p.m_It<&(*q);}
};
...
template<class T>
inline CPositionGroup2DConstIterator<T>& CPositionGroup2DConstIterator<T>::operator=(const CPositionGroup2DIterator<T>& It)
{
	m_It=&(*It);
	return *this;
}
