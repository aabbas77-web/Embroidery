...
bool kI_AllocImage(UINT32 Width, UINT32 Height,TIntImage* pInfo)
{
	...
	else
	{
		...
		k_ShowMessage(IPL_ERROR,&str,"kI_AllocImage() pInfo is a NULL pointer");
		k_EmptyString(&str);
		return false;
	}
}
...
bool kI_AllocImageFast(UINT32 Width, UINT32 Height,TIntImage* pInfo)
{
	...
	else
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kI_AllocImageFast() pInfo is a NULL pointer");
		k_EmptyString(&str);
		return false;
	}
}
...
bool kI_FlushImage(INT16 Color,TIntImage* pDest)
{
	...
	if (pDest==NULL)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kI_FlushImage() pDest is a NULL pointer");
		k_EmptyString(&str);
		return false;
	}
	else if (pDest->Origin==EMPTY)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_WARNING,&str,"kI_FlushImage() Image is empty - doing nothing");
		k_EmptyString(&str);
		return false;
	}
	else
	{
		...
		/* add comment to history */
		{
			...
			k_SprintfString(&History,"%skI_FlushImage(Color=%d)",ipl_HistoryIndent,Color);
...
bool kI_Load(const char* FileName, TIntImage* pInfo)
{
	...
	if (!k_SplitFileName(FileName, &Path,&FileNameExt,&Ext))
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kI_Load() Filename not valid");
		k_EmptyString(&str);
		return false;
	}
	else if (strcmp(Ext,"")==0)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kI_Load() No extension found");
		k_EmptyString(&str);
		returnValue=false;
	}
	else
	{
		...
		if (fp==NULL)
		{
			...
			k_InitString(&str);
			k_AddFileAndLine(str);
			k_ShowMessage(IPL_ERROR,&str,"kI_Load() Could not open file");
			k_EmptyString(&str);
			returnValue=false;
		}
		else
		...
		if (returnValue==true)
		{
			...
			/* add comment to history */
			{
				TString History;
				k_InitString(&History);
				k_SprintfString(&History,"%skI_Load(%s)",ipl_HistoryIndent,FileName);
...
bool kI_Save(const char* FileName, TIntImage* pInfo)
{
	...
	if (!k_SplitFileName(FileName, &Path,&FileNameExt,&Ext))
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kI_Save() Filename not valid");
		k_EmptyString(&str);
		return false;
	}
	else if (strcmp(Ext,"")==0)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kI_Save() No extension found");
		k_EmptyString(&str);
		returnValue=false;
	}
	else
	{
		...
		if (fp==NULL)
		{
			...
			k_InitString(&str);
			k_AddFileAndLine(str);
			k_ShowMessage(IPL_ERROR,&str,"kI_Save() Could not save file");
			k_EmptyString(&str);
			returnValue=false;
		}
		else if (pInfo->Origin==EMPTY)
		{
			...
			k_InitString(&str);
			k_AddFileAndLine(str);
			k_ShowMessage(IPL_ERROR,&str,"kI_Save() No image - cannot save");
			k_EmptyString(&str);
			returnValue=false;
		}
		else
		{
			...
			/* add comment to history */
			{
				TString History;
				k_InitString(&History);
				k_SprintfString(&History,"%skI_Save(%s)",ipl_HistoryIndent,FileName);
				k_AppendTextIPL(&(pInfo->History),History.pChars);
				k_EmptyString(&History);
			}
#endif
...
bool kI_SetPeriodicBorder(unsigned short BorderSize,TIntImage* pInfo)
{
	if (pInfo->Origin==EMPTY)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kI_SetPeriodicBorder() Image is empty");
		k_EmptyString(&str);
		return false;
	}
	else if (((unsigned int)BorderSize>pInfo->Width) || ((unsigned int)BorderSize>pInfo->Height))
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kI_SetPeriodicBorder() Border too large");
		k_EmptyString(&str);
		return false;
	}
	else
...
bool kI_CopyImage(TIntImage* pDest,const TIntImage* pSource)
{
	if (pDest==pSource)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kI_CopyImage() Source and destination image"
	...
	if (pSource->Origin!=EMPTY)
	{
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%skI_CopyImage() Source image file info: %s"
				  ...
				  TIntImage* pDest,const TIntImage* pSource)
{
	...
	if (pSource->Origin==EMPTY)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_WARNING,&str,"kI_CopySubImage() Source image is empty - doing nothing");
		k_EmptyString(&str);
		return false;
	}
		...
		(xmax<=kI_GetMaxX(pSource)) && (ymax<=kI_GetMaxY(pSource)))
	{
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%skI_CopySubImage(xmin=%d,ymin=%d,xmax=%d,ymax=%d) Source image file info: %s"
	...
	else
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kI_CopySubImage() Illegal clipping rectangel");
		k_EmptyString(&str);
		return false;
	}
}
...
void kI_Rotate90(int Steps,TIntImage* pSource)
{
	...
	if (pSource->Origin==EMPTY)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_WARNING,&str,"kI_Rotate90() Image is empty - doing nothing");
		k_EmptyString(&str);
		return;
	}
	if (ActSteps==0)
	...
	/* add comment to history */
	{
		TString History;
		k_InitString(&History);
		k_SprintfString(&History,"%skI_Rotate90(Steps=%d)",ipl_HistoryIndent,Steps);
	...
	else if (ActSteps==2)
	{
		for(y=0; y<(int)pSource->Height; y++)
			for(x=0; x<(int)pSource->Width; x++)
				kI_SetPixelMacro(x,MaxYIndex-y,kI_GetPixelMacro(x,y,*pSource),NewImage);
	}
	else if (ActSteps==3)
	{
		for(y=0; y<(int)pSource->Height; y++)
...
void kI_FlipHorisontal(TIntImage* pSource)
{
	...
	if (pSource->Origin==EMPTY)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_WARNING,&str,"kI_FlipHorisontal() Image is empty - doing nothing");
		k_EmptyString(&str);
		return;
	}
	...
	/* add comment to history */
	{
		TString History;
		k_InitString(&History);
		k_SprintfString(&History,"%skI_FlipHorisontal()",ipl_HistoryIndent);
...
void kI_FlipVertical(TIntImage* pSource)
{
	...
	if (pSource->Origin==EMPTY)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_WARNING,&str,"kI_FlipVertical() Image is empty - doing nothing");
		k_EmptyString(&str);
		return;
	}
	...
	/* add comment to history */
	{
		TString History;
		k_InitString(&History);
		k_SprintfString(&History,"%skI_FlipVertical()",ipl_HistoryIndent);
		k_AppendTextIPL(&(NewImage.History),History.pChars);
		k_EmptyString(&History);
		k_IplStartHistoryMacro();
	}
...
{ 
	if (pImg->Origin!=EMPTY)
	{
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%skI_ApplyBias() Bias=%g",ipl_HistoryIndent,Bias);
	...
	else
	{
        ...
        k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kI_ApplyBias: Source image empty");
		k_EmptyString(&str);
		return false;
	}
}
...
{ 
	if (pImg->Origin!=EMPTY)
	{
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%skI_ScaleIntensity() Prefactor=%g",ipl_HistoryIndent,Prefactor);
			k_AppendTextIPL(&(pImg->History),History.pChars);
			k_EmptyString(&History);
			k_IplStartHistoryMacro();
		}
	...
	else
	{
        ...
        k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kI_ScaleIntensity: Source image empty");
		k_EmptyString(&str);
		return false;
	}
}
...
bool kI_Add(TIntImage *pImg,TIntImage *pAddend)
{
		...
		|| (pImg->Width!=pAddend->Width) || (pImg->Height!=pAddend->Height))
	{
        ...
        k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kI_Add(): Images are empty or of different size");
		k_EmptyString(&str);
		return false;
	}
	else
	{
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%skI_Add() Adding a TIntImage",ipl_HistoryIndent);
			k_AppendTextIPL(&(pImg->History),History.pChars);
			k_EmptyString(&History);
			k_IplStartHistoryMacro();
		}
...
bool kI_Subtract(TIntImage *pImg,TIntImage *pSubtrahend)
{
		...
		|| (pImg->Width!=pSubtrahend->Width) || (pImg->Height!=pSubtrahend->Height))
	{
        ...
        k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kI_Subtract(): Images are empty or of different size");
		k_EmptyString(&str);
		return false;
	}
	else
	{
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%skI_Subtract() subtracting a TIntImage",ipl_HistoryIndent);
			k_AppendTextIPL(&(pImg->History),History.pChars);
			k_EmptyString(&History);
			k_IplStartHistoryMacro();
		}
...
bool kI_Multiply(TIntImage *pImg,TIntImage *pFactor)
{
		...
		|| (pImg->Width!=pFactor->Width) || (pImg->Height!=pFactor->Height))
	{
        ...
        k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kI_Multiply(): Images are empty or of different size");
		k_EmptyString(&str);
		return false;
	}
	else
	{
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%skI_Multiply() multiplying a TIntImage",ipl_HistoryIndent);
			k_AppendTextIPL(&(pImg->History),History.pChars);
			k_EmptyString(&History);
			k_IplStartHistoryMacro();
		}
...
bool kI_CopyDisplace(int dx,int dy, TIntImage *pImg)
{
	if (pImg->Origin!=EMPTY)
	{
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%skI_CopyDisplace() dx=%d dy=%d",ipl_HistoryIndent,dx,dy);
	...
	else
	{
        ...
        k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kI_CopyDisplace(): Image is empty");
		k_EmptyString(&str);
		return false;
	}
}
...
bool kI_CutOrZeroPad(TIntImage*pDest,TIntImage *pSource)
{
		...
		(ws>0) && (hs>0) && (wd>0) && (hd>0) && (pSource!=pDest))
	{
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%skI_CutOrZeroPad()",ipl_HistoryIndent);
			k_AppendTextIPL(&(pDest->History),History.pChars);
			k_EmptyString(&History);
			k_IplStartHistoryMacro();
		}
	...
	else
	{
		...
		if (pSource==pDest)
		{
			k_ShowMessage(IPL_ERROR,&str,"kI_CutOrZeroPad(): Cannot use same image as dest/source"); 
		}
 		else if ((ws<=0) || (hs<=0) || (wd<=0) || (hd<=0))
		{
			k_ShowMessage(IPL_ERROR,&str,"kI_CutOrZeroPad(): One or both images have no pixels"); 
		}
		else
		{
			k_ShowMessage(IPL_ERROR,&str,"kI_CutOrZeroPad(): One or both images are empty");
		}
		k_EmptyString(&str);
		return false;
	}
...
bool kI_CopyFromByte(TIntImage *pDest,TImage *pSource)
{
	if (pSource->Origin==EMPTY)
	{
        ...
        k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kI_CopyFromByte(): Image is empty");
		k_EmptyString(&str);
		k_EmptyString(&str);
		return false;
	}
	else
	{
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%skI_CopyFromByte()",ipl_HistoryIndent);
			k_AppendTextIPL(&(pDest->History),History.pChars);
			k_EmptyString(&History);
			k_IplStartHistoryMacro();
		}
...
bool kI_CopyFromFloat(TIntImage *pDest,TFloatImage *pSource)
{
	if (pSource->Origin==EMPTY)
	{
        ...
        k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kI_CopyFromFloat(): Image is empty");
		k_EmptyString(&str);
		k_EmptyString(&str);
		return false;
	}
	else
	{
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%skI_CopyFromFloat()",ipl_HistoryIndent);
			k_AppendTextIPL(&(pDest->History),History.pChars);
			k_EmptyString(&History);
			k_IplStartHistoryMacro();
		}
...
bool kI_CopyFromComplex(COMPLEXCHOISE mode, TIntImage *pDest,TComplexImage *pSource)
{
	if (pSource->Origin==EMPTY)
	{
        ...
        k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kI_CopyFromComplex(): Image is empty");
		k_EmptyString(&str);
		k_EmptyString(&str);
		return false;
	}
	else
	{
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%skI_CopyFromComplex()",ipl_HistoryIndent);
			k_AppendTextIPL(&(pDest->History),History.pChars);
			k_EmptyString(&History);
			k_IplStartHistoryMacro();
		}
...
bool kI_DrawLine(T2DInt Start, T2DInt End, INT16 Value, TIntImage *pDest)
{
	...
	if (pDest==NULL)
	{
        ...
        k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kI_DrawLine(): Image is empty");
		k_EmptyString(&str);
		return false;
	}
	k_InitGroup(&grp);
...
bool kI_DrawCircle(T2DInt Center, unsigned int Radius, INT16 Value, TIntImage *pDest)
{
	...
	if (pDest==NULL)
	{
        ...
        k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kI_DrawCircle(): Image is empty");
		k_EmptyString(&str);
		return false;
	}
	k_InitGroup(&grp);
...
INT16 kI_GetPixel(int x, int y,const TIntImage* pInfo)
{
	...
	else
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kI_GetPixel(%d,%d) Outside image and border",x,y);
		k_EmptyString(&str);
		return 0;
	}
}
...
bool kI_SetPixel(int x, int y, INT16 Color,TIntImage* pInfo)
{
	...
	else
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kI_SetPixel(%d,%d) Outside image and border",x,y);
		k_EmptyString(&str);
		return false;
	}
}
...
float kI_GetPixelInterpolated(float x, float y, const TIntImage* pInfo)
{
	...
	else
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kI_GetPixelInterpolated(%.2f,%.2f) Needs "
...
bool kI_LoadInt(FILE* fp,TIntImage* pInfo)
{
	...
	while (fgetc(fp)=='#')
	{
		if ((fgetc(fp)!='#') && (TokenRead==false))
		{
			...
			if (CharsInLine==-1)
			{
				...
				k_InitString(&str);
				k_AddFileAndLine(str);
				k_ShowMessage(IPL_ERROR,&str,"kI_LoadFloat() wrong header - "
		...
		else
		{
			...
			if (Token==NULL)
			{
				...
				k_InitString(&str);
				k_AddFileAndLine(str);
				k_ShowMessage(IPL_ERROR,&str,"kI_LoadInt() wrong file format, end of file reached");
				k_EmptyString(&str);
				return false;
			}
			else if (strcmp(Token,"WidthHeight")==0)
			{
				...
				if (fscanf(fp,"%d %d",&Width,&Height)!=2)
				{
					...
					k_InitString(&str);
					k_AddFileAndLine(str);
					k_ShowMessage(IPL_ERROR,&str,"kI_LoadInt() wrong header - "
				...
				if (WidthHeightRead==true)
				{
					...
					if ((Width!=Cols) || (Height!=Rows))
					{
						...
						k_InitString(&str);
						k_AddFileAndLine(str);
						k_ShowMessage(IPL_ERROR,&str,"kI_LoadInt() Width=Cols (%d=%d) and height=Rows (%d,%d) "
	...
	if (WidthHeightRead==false)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"kI_LoadInt() wrong header - width and height not found");
		k_EmptyString(&str);
		return false;
	}
	/* allocate image */
		...
		for(x=0;x<Width;x++)
		{
			if (k_SkipWhiteSpace(fp)!=-1)
			{
				...
				else
				{
					...
					k_InitString(&str);
					k_AddFileAndLine(str);
					k_ShowMessage(IPL_ERROR,&str,"kI_LoadInt() Failed reading image data");
					k_EmptyString(&str);
					kI_EmptyImage(pInfo);
					return false;
				}
...
bool kI_SaveInt(FILE* fp,TIntImage* pInfo)
{
	...
	if (pInfo->Origin==EMPTY)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_WARNING,&str,"kI_SaveInt() No image to save");
		k_EmptyString(&str);
		fclose(fp);
		return false;
	}
