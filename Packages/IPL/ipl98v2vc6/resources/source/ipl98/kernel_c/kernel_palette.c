		...
		default:
			{
				...
				k_ShowMessage(IPL_ERROR,&str,"k_CreatePal() Only supports 1,8 and 24 bit per pixels");
				k_EmptyString(&str);
				return false;
			}
	}
...
bool k_GetPalValue(int Index, UINT32* pPalValue, const TPalette* pPal)
{
	...
	else
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_GetPalValue() Index=%d outside range of palette",Index);
		k_EmptyString(&str);
		return false;
	}
}
...
UINT32 k_GetPalColor(int Index, const TPalette* pPal)
{
	...
	else
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_GetPalColor() Index=%d outside range of palette",Index);
		k_EmptyString(&str);
		return false;
	}
}
...
bool k_SetPalValue(int Index, UINT32 PalValue, TPalette* pPal)
{
	...
	else
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_SetPalValue() Index=%d outside range of palette",Index);
		k_EmptyString(&str);
		return false;
	}
}
...
bool k_CopyPalette(TPalette* pDest,const TPalette* pSource)
{
	if (pDest==pSource)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_CopyPalette() Source and destination palette"
...
bool k_IsPaletteGrayTone(const TPalette* pPal)
{
	...
	else
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_IsPaletteGrayTone() Number of palette intries is %d (must be 2 or 256)",
			pPal->PalEntries);
		k_EmptyString(&str);
		return false;
	}
