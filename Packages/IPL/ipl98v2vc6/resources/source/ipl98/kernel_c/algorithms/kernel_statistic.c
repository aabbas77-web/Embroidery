...
bool k_DeriveFloatImageStats(TStatistic* pStatistic,const TFloatImage* pImage)
{
	if (pImage->Origin==EMPTY)
	{
		...
		k_ShowMessage(IPL_ERROR,&str,"k_DeriveFloatImageStats() Image is empty");
	    k_EmptyString(&str);
		return false;
	}
	else
...
bool k_DeriveIntImageStats(TStatistic* pStatistic,const TIntImage* pImage)
{
	if (pImage->Origin==EMPTY)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_DeriveIntImageStats() Image is empty");
	    k_EmptyString(&str);
		return false;
	}
	else
...
bool k_DeriveImageStats(TStatistic* pStatistic,const TImage* pImage)
{
	...
	if (pImage->Origin==EMPTY)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_DeriveImageStats() Image is empty");
	    k_EmptyString(&str);
		return false;
	}
	/* allocate histogram */
	...
	else /* either 8 or 24 b/p */
	{
		...
		else
		{
			...
			k_InitString(&str);
			k_AddFileAndLine(str);
			k_ShowMessage(IPL_ERROR,&str,"k_DeriveImageStats() Image with %d b/p not supported",pImage->Bits);
			k_EmptyString(&str);
			return false;
		}
	}
...
bool k_DerivePixelGroupStats(TStatistic* pStatistic,const TPixelGroup* pPixelGroup)
{
	if (pPixelGroup->NumberOfPixels==0)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_DerivePixelGroupStats() No positions available");
	    k_EmptyString(&str);
		return false;
	}
	else if (k_ColorsAvailableGroup(pPixelGroup)==false)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_DerivePixelGroupStats() No color values available");
	    k_EmptyString(&str);
		return false;
	}
	else
