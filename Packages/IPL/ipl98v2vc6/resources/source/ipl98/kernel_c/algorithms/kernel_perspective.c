...
double k_RadialCalibAndGetMeanError(T3D2DPoints *pPointSets,double k,FLOAT32 CenterX,FLOAT32 CenterY);
...
double k_CalibGoldenSearch(double ax,double bx,double cx,double tol,double *xmin,
						  T3D2DPoints* pPointSets, FLOAT32 CenterX,FLOAT32 CenterY);
...
{ 
	...
	if ((pPointSets==NULL) || (pMat==NULL))
	{
		...
		k_ShowMessage(IPL_ERROR,&str,"k_CalibrateCamera() One of the parameters is a NULL pointer");
		k_EmptyString(&str);
		return false;
	}
	...
	if (pPointSets->NumberOfSets!=k_GetTotal3D2DPointSetsInUse(pPointSets))
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_CalibrateCamera() The T3D2DPoints structure containes entries "
	...
	if (pPointSets->NumberOfSets<6)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_CalibrateCamera() Needs at least 6 points (only %d is given)",pPointSets->NumberOfSets);
		k_EmptyString(&str);
		return false;
	}
...
{ 
	...
	if ((pWorld==NULL) || (pCcd==NULL) || (pMat==NULL))
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_Calibrate() One of the parameters is a NULL pointer");
		k_EmptyString(&str);
		return false;
	}
	nc=2*np;
	if (np<6)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_Calibrate() Needs at least 6 points (only %d is given)",nc);
		k_EmptyString(&str);
		return false;
	}
	a=(double**)calloc(11,sizeof(double*));
...
bool k_CalibrateWithRadialDist(T3D2DPoints* pPointSets,TCameraMatrix* pMat, 
							   double* k, FLOAT32 CenterX, FLOAT32 CenterY)
{
	...
	else if (pPointSets->NumberOfSets!=k_GetTotal3D2DPointSetsInUse(pPointSets))
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_CalibrateWithRadialDist() The T3D2DPoints structure containes entries "
			"where either the 2D or 3D point is not used");
		k_EmptyString(&str);
		return false;
	}
	...
	else if (pPointSets->NumberOfSets<6)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_CalibrateWithRadialDist() Need 6 refernce marks "
			"(current is %d)",pPointSets->NumberOfSets);
		k_EmptyString(&str);
		return false;
	}
	else
	{
	  ...
	  /*const double UPPER_VAL=0.000001;*/ /* 1e-6 */
		...
		if (iterate>=ITERATIONS)
		{
			...
			k_InitString(&str);
			k_AddFileAndLine(str);
			k_ShowMessage(IPL_WARNING,&str,"k_CalibrateWithRadialDist() Couldn't find a suitable"
...
bool k_RemoveRadialDistortion(TImage* pDest, TImage* pSource, double k, bool PreserveImageSize)
{
	...
	{
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_WARNING,&str,"k_RemoveRadialDistortion() Source image is empty - doing nothing");
		ReturnValue=false;
	}
	else if (pSource->Bits!=8)
	{
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_RemoveRadialDistortion() Image must be 8 b/p (current is %d b/p)",
			pSource->Bits);
		ReturnValue=false;
	}
...
bool k_MatrixToParameters(const TCameraMatrix* pMat, TCameraParameters* pPar)
{
        ...
        if(TrueSolutions == 0)
        {
                ...
                k_InitString(&str);
                k_AddFileAndLine(str);
                k_ShowMessage(IPL_ERROR,&str,"k_MatrixToParameters() No true solution found for angle a and b");
                k_EmptyString(&str);
                return false;
        }
        if(TrueSolutions > 1)
        {
                ...
                k_InitString(&str);
                k_AddFileAndLine(str);
                k_ShowMessage(IPL_WARNING,&str,"k_MatrixToParameters() More solutions found for angle a and b");
                k_EmptyString(&str);
        }
        ...
        if(TrueSolutions == 0)
        {
                ...
                k_InitString(&str);
                k_AddFileAndLine(str);
                k_ShowMessage(IPL_ERROR,&str,"k_MatrixToParameters() No true solution found for angle c");
                k_EmptyString(&str);
                return false;
        }
        if(TrueSolutions > 1)
        {
                ...
                k_InitString(&str);
                k_AddFileAndLine(str);
                k_ShowMessage(IPL_WARNING,&str,"k_MatrixToParameters() More solutions found for angle c");
                k_EmptyString(&str);
        }
        ...
        if(TrueSolutions == 0)
        {
                ...
                k_InitString(&str);
                k_AddFileAndLine(str);
                k_ShowMessage(IPL_ERROR,&str,"k_MatrixToParameters() No true solution found for angle beta");
                k_EmptyString(&str);
                return false;
        }
        if(TrueSolutions > 1)
        {
                ...
                k_InitString(&str);
                k_AddFileAndLine(str);
                k_ShowMessage(IPL_WARNING,&str,"k_MatrixToParameters() More solutions found for angle beta");
                k_EmptyString(&str);
        }
        return true;
}
...
bool k_ParametersToMatrix(const TCameraParameters* pPar, TCameraMatrix* pMat)
{
        ...
        if(pPar->f==0.0)
        {
                ...
                k_InitString(&str);
                k_AddFileAndLine(str);
                k_ShowMessage(IPL_ERROR,&str,"k_ParametersToMatrix() Camera matrix undefined for f=0.0");
                k_EmptyString(&str);
                return false;
        }
        if((pPar->dx==0.0) && (pPar->dy==0.0) && (pPar->dz==0.0))
        {
                ...
                k_InitString(&str);
                k_AddFileAndLine(str);
                k_ShowMessage(IPL_ERROR,&str,"k_ParametersToMatrix() Camera matrix undefined for dx=0.0, dy=0.0 and dz=0.0");
                k_EmptyString(&str);
                return false;
        }
        if((pPar->b==PI) && (pPar->dx==0.0))
        {
                ...
                k_InitString(&str);
                k_AddFileAndLine(str);
                k_ShowMessage(IPL_ERROR,&str,"k_ParametersToMatrix() Camera matrix undefined for b=Pi and dx=0.0o");
                k_EmptyString(&str);
                return false;
        }
...
#define CALIB_SHFT2(a,b,c) (a)=(b);(b)=(c);
#define CALIB_SHFT3(a,b,c,d) (a)=(b);(b)=(c);(c)=(d);
double k_CalibGoldenSearch(double ax,double bx,double cx,double tol,double *xmin,
						  T3D2DPoints* pPointSets, FLOAT32 CenterX, FLOAT32 CenterY)
{
