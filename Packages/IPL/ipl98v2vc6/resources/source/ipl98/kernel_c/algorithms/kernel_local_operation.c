...
{ 
	if ((pSource->Origin==EMPTY) || (pSource==pDest) || (pSource->Bits==1))
	{
		...
		if (pSource->Origin==EMPTY)
			k_ShowMessage(IPL_ERROR,&str,"k_DrawHistogram() Source image is empty");
		else if (pSource==pDest)
			k_ShowMessage(IPL_ERROR,&str,"k_DrawHistogram() Source and destination image cannot be the same");
		else
			k_ShowMessage(IPL_ERROR,&str,"k_DrawHistogram() Source has 1 b/p");
	...
	else
	{
		...
		/* add comment to history */
		{
			...
			k_SprintfString(&History,"%sk_DrawHistogram() Drawn from image with file info: %s",
...
bool k_EqualizeHistogram(TImage *pSource,TImage *pDest)
{
	if ((pSource->Origin==EMPTY) || (pSource->Bits<8))
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_EqualizeHistogram() Source image is empty or image is not 8 or 24 b/p");
		k_EmptyString(&str);
		return false;
	} 
	else
	{
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%sk_EqualizeHistogram()",ipl_HistoryIndent);
							   ...
							   int Min,int Max, TImage *pDest)
{
	if (pSource->Bits!=8)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_NonLinearByteToByte(). Source image does not have 8 b/p");
		k_EmptyString(&str);
		return false;
	}
	else if ((pSource->Origin==EMPTY) || (Max<=Min) || (gamma<0))
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_NonLinearCopyByteToByte(). Source image is empty or wrong"
			" input data: gamma= %f, Min = %d, Max = %d", gamma, Min,Max);
		k_EmptyString(&str);
		return false;
	}
	else
	{ 
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%sk_NonLinearByteToByte() gamma=%f, Min=%d, Max=%d",
							  ...
							  int Min,int Max, TImage *pDest)
{
	if ((pSource->Origin==EMPTY) || (Max<=Min) || (gamma<0))
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_NonLinearCopyIntToByte(). Source image is empty or wrong"
			" input data: gamma= %f, Min = %d, Max = %d", gamma, Min,Max);
		k_EmptyString(&str);
		return false;
	}
	...
	{ 
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%sk_NonLinearIntToByte() gamma=%f, Min=%d, Max=%d",
				ipl_HistoryIndent,gamma,Min,Max);
			k_AppendTextIPL(&(pDest->History),History.pChars);
			k_EmptyString(&History);
			k_IplStartHistoryMacro();
								...
								float Min, float Max, TImage *pDest)
{
	if ((pSource->Origin==EMPTY) && (Max<=Min) && (gamma<0))
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_NonLinearCopyFloatToByte(). Source image is empty or wrong"
			" input data: gamma= %f, Min = %f, Max = %f", gamma, Min,Max);
		k_EmptyString(&str);
		return false;
	}
	else
	{ 
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%sk_NonLinearCopyFloatToByte() gamma=%f, Min=%f, Max=%f",
				ipl_HistoryIndent,gamma,Min,Max);
			k_AppendTextIPL(&(pDest->History),History.pChars);
			k_EmptyString(&History);
			k_IplStartHistoryMacro();
								  ...
								  float Min, float Max, TImage *pDest)
{
	if ((pSource->Origin==EMPTY) || (Max<=Min) || (gamma<0))
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_NonLinearComplexToByte(). Source image is empty or wrong"
			" input data: gamma= %f, Min = %f, Max = %f", gamma, Min,Max);
		k_EmptyString(&str);
		return false;
	}
	...
	{ 
		...
		/* add comment to history */
		{
			TString History;
			k_InitString(&History);
			k_SprintfString(&History,"%sk_NonLinearCopyComplexToByte() gamma=%f, Min=%f, Max=%f",
				ipl_HistoryIndent,gamma,Min,Max);
			k_AppendTextIPL(&(pDest->History),History.pChars);
			k_EmptyString(&History);
			k_IplStartHistoryMacro();
