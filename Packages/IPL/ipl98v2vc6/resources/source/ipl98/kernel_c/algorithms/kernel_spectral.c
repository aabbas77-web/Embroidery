...
factor }*/
{
	...
	if (pImg->Origin!=EMPTY)
	{
			...
			((int)(PowerY+0.00001)-PowerY<0.00001))
		{
			...
			/* add comment to history */
			{
				...
				k_SprintfString(&History,"%sk_FFT() isign=%d",ipl_HistoryIndent,isign);
	...
	else
    {
		...
		k_ShowMessage(IPL_ERROR,&str,"k_FFT(): Height or width of Complex image is not a power of 2");
		k_EmptyString(&str);
		return false;
	}
} 
  else
  {
	  ...
	  k_InitString(&str);
	  k_AddFileAndLine(str);
	  k_ShowMessage(IPL_ERROR,&str,"k_FFT(): Image is empty");
	  k_EmptyString(&str);
	  return false;
  }
}
