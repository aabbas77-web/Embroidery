						   ...
						   TPixelGroups* pPixelGroups)
{
	...
	if (Index>=pPixelGroups->NumberOfGroups)
	{
		...
		k_ShowMessage(IPL_ERROR,&str,"k_InsertGroupInGroups() Index (%d) is out of range (NumberOfGroups=%d)",
...
bool k_AddColorsToGroups(const TImage* pSource,TPixelGroups* pPixelGroups)
{
	...
	if ((pSource==NULL) || (pPixelGroups==NULL))
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_AddColorsToGroups() At least one parameter is a NULL pointer");
	    k_EmptyString(&str);
		return false;
	}
	for(i=0; i<pPixelGroups->NumberOfGroups; i++)
...
bool k_RemoveGroup(TPixelGroups *pPixelGroups,unsigned int GroupIndex)
{
	if (pPixelGroups==NULL)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_RemoveGroup() pPixelGroups parameter is NULL");
	    k_EmptyString(&str);
		return false;
	}
	if (GroupIndex>=pPixelGroups->NumberOfGroups)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_RemoveGroup() Blob index out of range");
	    k_EmptyString(&str);
		return false;
	}
...
bool k_RemoveGroupSlow(TPixelGroups *pPixelGroups,unsigned int GroupIndex)
{
	if (GroupIndex>=pPixelGroups->NumberOfGroups)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_RemoveGroupSlow() Index (%d) is out of range (NumberOfGroups=%d)",
					   ...
					   T2DInt Pos,unsigned int* pPosIndex)
{
	...
	if (pPixelGroups==NULL)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_GetGroupWithPos() pPixelGroup parameter is NULL");
	    k_EmptyString(&str);
		return false;
	}
	for (i=0;i<pPixelGroups->NumberOfGroups;i++)
...
void k_ReAllocGroups(TPixelGroups* pPixelGroups,unsigned int NewSize)
{
	...
	else
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_ReAllocGroups() Realloc failed");
	    k_EmptyString(&str);
	}
}
