...
bool k_AppendText(TIplText* pText,const char* pStr)
{
	...
	{
		TString ErrorStr;
		k_InitString(&ErrorStr);
		k_AddFileAndLine(ErrorStr);
		k_ShowMessage(IPL_ERROR,&ErrorStr,"CText::k_AppendText() String argument=\"%s\" contains "
			"a newline character - no insertion",pStr);
	    k_EmptyString(&ErrorStr);
		return false;
	}
...
bool k_AppendCPArgumentsText(TIplText* pText,int argc, const char** ppArgv)
{
	...
	if (ppArgv==NULL)
	{
		...
		k_ShowMessage(IPL_ERROR,&str,"k_AppendCPArgumentsText: ppArgv is a NULL pointer");
	    k_EmptyString(&str);
		return false;
	}
	k_InitString(&InsertStr);
...
bool k_CopyText(TIplText* pDest,const TIplText* pSource)
{
	if (pDest==pSource)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_CopyText() Source and destination text"
...
bool k_FindNextText(TIplText* pText, TString* pResult)
{
	...
	if (pText->SearchStr.NumberOfChars==0)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_FindNextText(): No search string present (call k_FindText() first)");
	    k_EmptyString(&str);
		return false;
	}
	while((Found==false) && (pText->FindPosition<pText->NumberOfLines))
...
bool k_GetItemText(const TIplText* pText,unsigned long Index,TString* pResult)
{
	if(Index>=pText->NumberOfLines)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_GetItemText(): Index(%d) out of range",Index);
	    k_EmptyString(&str);
		return false;
	}
	else
...
bool k_LoadText(TIplText* pText, const char* pFileName)
{
	...
	if (!fp)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_LoadText() Failed loading history file %s",pFileName);
	    k_EmptyString(&str);
		return false;
	}
	else
	{
		...
		while(fgets(pLine,LineSize-1,fp)!=NULL)
		{
			if (pLine[0]!='#')
			{
				...
				k_InitString(&str);
				k_AddFileAndLine(str);
				k_ShowMessage(IPL_ERROR,&str,"k_LoadText() Wrong format in history file (one "
					"of the lines contains more than %d characters or a '#' character is "
					"missing in the beginning of a line)",LineSize);
				fclose(fp);
				free(pLine);
				k_DeleteText(pText);
				k_EmptyString(&str);
...
bool k_SaveText(TIplText* pText, const char* pFileName)
{
	...
	if (pFileName==NULL)
	{
		if (pText->FileInfo.PathAndFileName==NULL)
		{
			...
			k_InitString(&str);
			k_AddFileAndLine(str);
			k_ShowMessage(IPL_ERROR,&str,"k_SaveText() Both the pFileName and FileInfo "
	...
	if (!fp)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_SaveText() Failed saving history file %s",pFileName);
	    k_EmptyString(&str);
		return false;
	}
	else
...
bool k_SetFileInfoFromPathFileText(TIplText* pText, const char* pPathAndFileName)
{
	...
	if (pPathAndFileName==NULL)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_SetFileInfoFromPathFileText() pPathAndFileName is a NULL pointer");
	    k_EmptyString(&str);
		return false;
	}
	...
	if (k_SplitFileName(PathAndFileName.pChars,&(pText->FileInfo.PathName),&(pText->FileInfo.FileName),&Ext)==false)
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_SetFileInfoFromPathFileText() Failed setting variables in TFileInfo structure");
	    k_EmptyString(&str);
...
void k_ReAllocText(TIplText* pText,unsigned int NewSize)
{
	...
	else
	{
		...
		k_InitString(&str);
		k_AddFileAndLine(str);
		k_ShowMessage(IPL_ERROR,&str,"k_ReAllocText() Realloc failed");
		k_EmptyString(&str);
	}
}
